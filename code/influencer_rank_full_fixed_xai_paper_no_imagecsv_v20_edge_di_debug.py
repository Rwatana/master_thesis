...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    def deletion_insertion_feature_v3(
        model,
        input_graphs,           # Jan..Nov (T個)
        target_node,
        baseline_score,
        ranked_feature_order,
        mode="deletion",
        random_trials=50,
        seed=0,
        device="cpu",
        ...
        ranked = [int(i) for i in ranked_feature_order if 0 <= int(i) < Fdim]
        ranked = list(dict.fromkeys(ranked))
        rest = [i for i in range(Fdim) if i not in set(ranked)]
        full_order = ranked + rest

        k_list = list(range(0, Fdim + 1))
        ...
        for k in k_list:
            if mode == "deletion":
                ...
                masked = full_order[k:]  # keep top-k
            else:
                raise ValueError(mode)
        ...
        for t in range(int(random_trials)):
            perm = rng.permutation(Fdim).tolist()
            for i, k in enumerate(k_list):
                masked = perm[:k] if mode == "deletion" else perm[k:]
                graphs_m = [apply_feature_mask_on_x_target(g, target_node, masked, mode=mask_mode)
                            for g in input_graphs]
                rand_scores[t, i] = predict(graphs_m)

        return k_list, np.array(scores_main, np.float32), rand_scores.mean(0), rand_scores.std(0), orig
    ...
    def deletion_insertion_edge_v3(
        ...
        target_node,
        baseline_score,
        ranked_neighbor_order,   # 重要順 neighbor id (node index)
        mode="deletion",         # "deletion" or "insertion"
        random_trials=50,
        seed=0,
        device="cpu",
        max_neighbors=30,        # ★0以下なら「全近傍まで」
        k_stride=1,              # ★kの間引き
        random_pool="all",       # "all" or "topK"
        fill_rest=False,         # ★Trueで「全近傍までのmain曲線」も作れる
    ):
        import copy
        import numpy as np
        import torch
        ...
        # insertion: incident以外は保持し、incidentは keep_neighbors だけ残す
        def apply_edge_keep_only_incident(graph, target_node: int, keep_neighbors):
            ...
            keep = ~((src == uu) | (dst == uu))  # incident以外は全部keep
        ...
        def predict(graphs):
            with torch.no_grad():
                ...
                for g in graphs:
                    ...
                    p_x = model.projection_layer(g.x)
                    gcn_out = model.gcn_encoder(p_x, g.edge_index)
                    raw_emb.append(p_x[u].unsqueeze(0))
                    seq_emb.append(gcn_out[u].unsqueeze(0))
                f_seq = torch.stack(seq_emb, dim=1)  # [1,T,D]
        ...
        if len(all_nbrs) == 0:
            orig = predict(input_graphs)
            return [0], np.array([orig], np.float32), np.array([orig], np.float32), np.array([0.0], np.float32), orig, 0

        all_set = set(all_nbrs)

        # ranked unique & in all_set
        ranked = []
        seen = set()
        ...
        if int(max_neighbors) <= 0:
            K_eval = len(main_order)
        else:
            K_eval = min(int(max_neighbors), len(main_order))

        top_order = main_order[:K_eval]

        # k_list with stride (ensure end included)
        k_list = list(range(0, K_eval + 1, max(1, int(k_stride))))
        if k_list[-1] != K_eval:
            k_list.append(K_eval)
        ...
        for t in range(int(random_trials)):
            ...
            perm_neighbors = [rand_pool[j] for j in perm]
    ...
    do_di = bool(params.get("DO_DELETION_INSERTION", True))
    di_random_trials = int(params.get("DI_RANDOM_TRIALS", 30))
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        with torch.no_grad():
            ...
            f_seq_all = torch.stack(seq_emb_l, dim=0).permute(1,0,2).contiguous()  # [Nall,T,D]
            f_raw_all = torch.stack(raw_emb_l, dim=0).permute(1,0,2).contiguous()  # [Nall,T,P]

            all_indices_cpu = all_indices.detach().cpu().long()  # [Ntest] global node ids
            f_seq = f_seq_all.index_select(0, all_indices_cpu)   # [Ntest,T,D]
            f_raw = f_raw_all.index_select(0, all_indices_cpu)   # [Ntest,T,P]
        ...
        try:
            ...
            for explain_pos in positions_to_explain:
                ...
                # -----------------------------
                # Deletion / Insertion curves using MaskOpt order
                # -----------------------------
                if do_di and (row_nz is not None):
                    # FEATURE order (must be x-dim indices)
                    # feat_order = []
                    # if df_feat is not None and (not df_feat.empty):
                    #     # try common column names
                    #     for col in ["feature_idx", "feat_idx", "feature_index", "idx"]:
                    #         if col in df_feat.columns:
                    #             # also try common importance column names
                    #             imp_col = None
                    #             for c2 in ["importance", "score_impact", "impact", "weight", "mask"]:
                    #                 if c2 in df_feat.columns:
                    #                     imp_col = c2
                    #                     break
                    #             if imp_col is None:
                    #                 # fallback: keep as given order
                    #                 feat_order = df_feat[col].astype(int).tolist()
                    #             else:
                    #                 feat_order = df_feat.sort_values("Importance", ascending=False)["feature_idx"].astype(int).tolist()
                    #             break

                    # # EDGE order (neighbor ids)
                    # edge_order = []
                    # if df_edge is not None and (not df_edge.empty):
                    #     for col in ["neighbor_id", "nbr_id", "dst", "src", "node_id", "neighbor"]:
                    #         if col in df_edge.columns:
                    #             imp_col = None
                    #             for c2 in ["importance", "score_impact", "impact", "weight", "mask"]:
                    #                 if c2 in df_edge.columns:
                    #                     imp_col = c2
                    #                     break
                    #             if imp_col is None:
                    #                 edge_order = df_edge[col].astype(int).tolist()
                    #             else:
                    #                 edge_order = df_edge.sort_values(imp_col, ascending=False)[col].astype(int).tolist()
                    #             break
                    ...
                    # --- inside: for explain_pos in positions_to_explain: の中 ---

                    # ===== FEATURE order =====
                    feat_order = []
                    if df_feat is not None and (not df_feat.empty):
                        feat_idx_col = _pick_col(df_feat, ["feature_idx", "feat_idx", "feature_index", "idx"])
                        imp_col = _pick_col(df_feat, ["importance", "score_impact", "impact", "weight", "mask"])

                        if feat_idx_col is not None:
                            if imp_col is not None:
                                ...
                                feat_order = df_feat[feat_idx_col].astype(int).tolist()

                        # もし idx 列が無く feature 名しかない場合の保険（必要なら）
                        elif "feature" in df_feat.columns:
                            # feature_names から index を引く
                            tmp = []
                            for nm in df_feat["feature"].astype(str).tolist():
                                if nm in feature_names:
                                    tmp.append(int(feature_names.index(nm)))
                            feat_order = tmp
                ...
                if len(edge_order) > 0:
                    k, main, rmean, rstd, orig, K = deletion_insertion_edge_v3(
                        ...
                        seed=0,
                        device=device,
                        max_neighbors=int(params.get("DI_EDGE_MAX_NEIGHBORS", 30)),
                        k_stride=int(params.get("DI_EDGE_K_STRIDE", 1)),
                        random_pool="all",
                        fill_rest=False,   # ★topKだけ
                    )
