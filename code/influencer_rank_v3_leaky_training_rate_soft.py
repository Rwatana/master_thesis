...
MODEL_SAVE_PATH = f'influencer_rank_model_{time.strftime("%Y%m%d")}_rich_features_2017_rate_soft.pth'
...
class InfluencerRankModel(nn.Module):
    def __init__(self, feature_dim, gcn_dim, rnn_dim, num_gcn_layers=2, dropout_prob=0.5):
        ...
        self.predictor = nn.Sequential(Linear(rnn_dim, 16), LeakyReLU(), Dropout(dropout_prob), Linear(16, 1), Softplus())
