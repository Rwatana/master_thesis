...
def _infer_edge_type(name: str, neighbor_id: int | None = None, influencer_id_set: set[int] | None = None):
    if name is None:
        return "unknown"
    s = str(name)
    ...
    # ★重要：名前に @ が付かない "ユーザノード" を mention と判定
    if (neighbor_id is not None) and (influencer_id_set is not None) and (int(neighbor_id) in influencer_id_set):
        return "mention"

    return "unknown"

def _candidate_node_keys(nm: str):
    s = str(nm).strip()
    if s == "":
        return []
    ...
    # object prefix variants
    if s.startswith("obj:"):
        cands.append("object:" + s[len("obj:"):])
    if s.startswith("object:"):
        cands.append("obj:" + s[len("object:"):])
...
def _name_to_neighbor_id(nm: str, node_to_idx: dict):
    for key in _candidate_node_keys(nm):
        if key in node_to_idx:
            return int(node_to_idx[key])
    return np.nan


def _normalize_edge_df(df_edge: pd.DataFrame, idx_to_node: dict, node_to_idx: dict,
                       influencer_id_set: set[int] | None = None):
    """
    MaskOpt df_edge を Streamlit用に正規化：
    ...
    out = pd.DataFrame({
        ...
        "importance": pd.to_numeric(d[imp_col], errors="coerce").astype(float),
    })

    # ★ここが重要：表記ゆれ吸収して neighbor_id を作る
    out["neighbor_id"] = out["neighbor_name"].map(lambda nm: _name_to_neighbor_id(nm, node_to_idx))
    ...
    if "Type" in d.columns:
        ...
        out["edge_type"] = [
            t if t in ["hashtag", "mention", "object"]
            else _infer_edge_type(nm, neighbor_id=nid, influencer_id_set=influencer_id_set)
            for t, nm, nid in zip(tmp, out["neighbor_name"].tolist(), out["neighbor_id"].tolist())
        ]
    else:
        out["edge_type"] = [
            _infer_edge_type(nm, neighbor_id=nid, influencer_id_set=influencer_id_set)
            for nm, nid in zip(out["neighbor_name"].tolist(), out["neighbor_id"].tolist())
        ]

    out = out.sort_values("importance", ascending=False).reset_index(drop=True)
    return out[["neighbor_id", "importance", "neighbor_name", "edge_type"]]
...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        try:
            ...
            influencer_id_set = set(int(x) for x in influencer_indices)
            ...
            for explain_pos in positions_to_explain:
                ...
                try:
                    df_feat, df_edge, meta = maskopt_e2e_explain(
                        ...
                        budget_edge=int(params.get("maskopt_budget_edge", 80)),
                ...
                # ★ここが重要：posの保存は必ずやる
                try:
                    df_edge_norm = _normalize_edge_df(df_edge, idx_to_node=idx_to_node, node_to_idx=node_to_idx, influencer_id_set=influencer_id_set)
