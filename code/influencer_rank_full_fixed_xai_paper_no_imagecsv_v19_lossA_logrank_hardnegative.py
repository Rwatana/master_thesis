...
class ListwiseTailBatchSampler(Sampler):
    """
    Make listwise batches for ListMLE:
      - each list (size=list_size) contains at least `right_per_list` right-tail samples
      - optionally fill the rest with hard negatives first, then random
    """
    def __init__(
        self,
        targets: np.ndarray,          # shape [N], train targets (true)
        list_size: int,
        batch_size: int,              # must be multiple of list_size
        right_q: float = 0.90,
        right_per_list: int = 1,
        hard_neg_q: float = 0.70,     # hard negatives: [q, thr_hi)
        num_batches: int | None = None,
        seed: int = 42,
        replacement: bool = True,
        log_stats: bool = False,
    ):
        assert batch_size % list_size == 0, "batch_size must be multiple of list_size"
        self.targets = targets.astype(float)
        self.N = len(self.targets)
        self.list_size = int(list_size)
        self.batch_size = int(batch_size)
        self.lists_per_batch = self.batch_size // self.list_size

        self.right_q = float(right_q)
        self.right_per_list = int(right_per_list)
        self.hard_neg_q = float(hard_neg_q)
        ...
        # thresholds
        self.thr_hi = float(np.quantile(self.targets, self.right_q))
        self.thr_hard = float(np.quantile(self.targets, self.hard_neg_q))

        idx_all = np.arange(self.N)
        self.idx_right = idx_all[self.targets >= self.thr_hi]
        self.idx_hard  = idx_all[(self.targets >= self.thr_hard) & (self.targets < self.thr_hi)]
        self.idx_rest  = idx_all[self.targets < self.thr_hard]

        if len(self.idx_right) == 0:
            raise RuntimeError("No right-tail samples found. Check RIGHT_Q / targets distribution.")
        if self.right_per_list > self.list_size:
            raise RuntimeError("right_per_list must be <= list_size")

        # how many batches per epoch
        if num_batches is None:
            # roughly one pass worth; you can change this freely
            self.num_batches = math.ceil(self.N / self.batch_size)
        else:
    ...
    def _choice(self, pool: np.ndarray, k: int):
        if k <= 0:
        ...
        # without replacement: if insufficient, fallback to replacement for the remainder
        if k <= len(pool):
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            # build lists
            for _l in range(self.lists_per_batch):
                # 1) pick right-tail
                tail_idx = self._choice(self.idx_right, self.right_per_list)

                # 2) fill remainder: hard negatives first, then rest
                remain = self.list_size - len(tail_idx)
                hard_take = min(remain, max(0, len(self.idx_hard)))  # cap
                hard_idx = self._choice(self.idx_hard, hard_take)

                remain2 = remain - len(hard_idx)
                rest_pool = self.idx_rest if len(self.idx_rest) > 0 else np.setdiff1d(np.arange(self.N), self.idx_right)
                rest_idx = self._choice(rest_pool, remain2)

                one_list = np.concatenate([tail_idx, hard_idx, rest_idx]).astype(np.int64)
                # shuffle within list
                self.rng.shuffle(one_list)
                batch_indices.append(one_list)

            batch_indices = np.concatenate(batch_indices).tolist()

            if self.log_stats:
                # optional: quick sanity check each yielded batch
                ys = self.targets[np.array(batch_indices)]
                # count right-tail per list
                ys = ys.reshape(self.lists_per_batch, self.list_size)
                right_counts = (ys >= self.thr_hi).sum(axis=1)
                # you can print or log these
                # print("right_counts(min/mean/max)=", right_counts.min(), right_counts.mean(), right_counts.max())
...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        if mode_run != "infer":
            ...
            # ---- listwise-tail batch sampler ----
            RIGHT_Q = float(params.get("RIGHT_Q", 0.90))
            RIGHT_PER_LIST = int(params.get("RIGHT_PER_LIST", 1))   # new param
            HARD_NEG_Q = float(params.get("HARD_NEG_Q", 0.70))      # new param

            y_all = train_dataset.tensors[1].detach().cpu().numpy().astype(float)

            batch_sampler = ListwiseTailBatchSampler(
                targets=y_all,
                list_size=list_size,
                batch_size=safe_bs,
                right_q=RIGHT_Q,
                right_per_list=RIGHT_PER_LIST,
                hard_neg_q=HARD_NEG_Q,
                num_batches=None,          # 1 epoch ≈ 1 pass
                seed=42,
                replacement=True,
                log_stats=False,
            )
            ...
            for epoch in range(params["EPOCHS"]):
                ...
                for batch in dataloader:
                    ...
                    TAIL_Q = float(params.get("TAIL_Q", 0.90))
                    TAIL_WEIGHT = float(params.get("TAIL_WEIGHT", 5.0))

                    # b_target: [B], preds: [B]
                    thr = torch.quantile(b_target.detach(), TAIL_Q)

                    w = torch.ones_like(b_target)
                    w = torch.where(
                        b_target >= thr,
                        b_target.new_full((), TAIL_WEIGHT),  # scalar tensor on same device/dtype
                        w
                    )
...
def main():
    ...
    sweep_grid = {
        ...
        # 'POINTWISE_LOSS_WEIGHT': [0.01, 0.1, 0.5],
        # 'GCN_DIM': [64, 128],
        # 'RNN_DIM': [64, 128],
