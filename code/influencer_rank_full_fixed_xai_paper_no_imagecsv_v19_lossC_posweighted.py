        ...
        if mode_run != "infer":
            ...
            for epoch in range(params["EPOCHS"]):
                ...
                for batch in dataloader:
                    ...
                    # ✅ pointwise を正例重視で重み付け
                    thr = float(params.get("POS_THRESHOLD", 0.01))      # engagement がこの値以上を「正例」とみなす
                    pos_w = float(params.get("POS_WEIGHT", 5.0))        # 正例の重み倍率

                    w = torch.ones_like(b_target, dtype=log_pred.dtype, device=log_pred.device)
                    w = w + (b_target > thr).to(w.dtype) * (pos_w - 1.0)
                    w = w / w.mean().clamp_min(1e-12)  # スケール暴れ防止

                    loss_point = (w * (log_pred - log_target) ** 2).mean()
