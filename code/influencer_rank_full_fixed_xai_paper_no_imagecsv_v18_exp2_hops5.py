...
# ===================== Deletion / Insertion (Counterfactual) Eval =====================
def _di_make_k_list(n_items: int, k_max: int, k_step: int):
    n_items = int(max(0, n_items))
    if n_items == 0:
        return [0]
    km = int(min(max(1, k_max), n_items))
    ks = list(range(0, km + 1, int(max(1, k_step))))
    if ks[-1] != km:
        ks.append(km)
    return ks

def _di_auc(x, y):
    # x: fraction in [0,1], y: curve
...
@torch.no_grad()
def _di_predict(wrapper, feat_gate, edge_gate, cudnn_ctx=None):
    if cudnn_ctx is None:
        class _Null:
            def __enter__(self): return None
            def __exit__(self, exc_type, exc, tb): return False
        cudnn_ctx = _Null()
    with cudnn_ctx:
        return float(wrapper.predict_with_gates(feat_gate, edge_gate).item())

def _di_get_order_feature(wrapper, feat_gate, strategy: str, rng: np.random.RandomState):
    Fdim = int(feat_gate.numel()) if feat_gate is not None else int(wrapper.feature_dim)
    if Fdim <= 0:
        return []
    g = feat_gate.detach().cpu().numpy().astype(float) if feat_gate is not None else np.ones(Fdim, dtype=float)
    base = np.argsort(g)[::-1]  # xai: gate high -> important
    ...
    if st == "random":
        return rng.permutation(Fdim).tolist()
    # fallback
    return base.tolist()

def _di_get_order_edge(wrapper, edge_gate, strategy: str, rng: np.random.RandomState):
    Edim = int(edge_gate.numel()) if edge_gate is not None else int(getattr(wrapper, "num_edge_params", 0))
    if Edim <= 0:
        return []
    g = edge_gate.detach().cpu().numpy().astype(float) if edge_gate is not None else np.ones(Edim, dtype=float)
    base = np.argsort(g)[::-1]
    st = str(strategy).lower()
    ...
    if st == "reverse":
        return base[::-1].tolist()
    if st == "random":
        return rng.permutation(Edim).tolist()

    # degree baseline (works best when edge_grouping="neighbor")
    if st == "degree":
        # compute degree on the *global* explain_pos graph
        g_exp = wrapper.input_graphs[wrapper.explain_pos]
        ei = g_exp.edge_index
        num_nodes = int(g_exp.num_nodes)
        # undirected edge_index is used in your pipeline, but either side is fine for relative ranking
        deg = degree(ei[0], num_nodes=num_nodes).detach().cpu().numpy().astype(float)

        # if grouped-by-neighbor, we have neighbor_global per group
        meta = getattr(wrapper, "edge_group_meta", None)
        if meta is not None and len(meta) == Edim and "neighbor_global" in meta[0]:
            pairs = []
            for gid in range(Edim):
                ng = int(meta[gid].get("neighbor_global", -1))
                d = float(deg[ng]) if 0 <= ng < len(deg) else 0.0
                pairs.append((gid, d))
            pairs.sort(key=lambda x: x[1], reverse=True)
            return [gid for gid, _ in pairs]

        # fallback: if not grouped, degree baseline is not well-defined -> fallback to xai
        return base.tolist()

    # fallback
    return base.tolist()

def _di_eval_kind_curves(
    wrapper,
    ...
    kind = str(kind).lower()
    mode = str(mode).lower()
    assert kind in ("feature", "edge")
    assert mode in ("deletion", "insertion")

    Fdim = int(wrapper.feature_dim)
    Edim = int(getattr(wrapper, "num_edge_params", 0))

    ones_feat  = torch.ones(Fdim, device=wrapper.device)
    zeros_feat = torch.zeros(Fdim, device=wrapper.device)
    ones_edge  = torch.ones(Edim, device=wrapper.device) if Edim > 0 else None
    zeros_edge = torch.zeros(Edim, device=wrapper.device) if Edim > 0 else None

    pred_full = _di_predict(wrapper, ones_feat, ones_edge, cudnn_ctx=cudnn_ctx)

    if kind == "feature":
        # baseline for insertion: remove all target features (but keep edges)
        pred_base = _di_predict(wrapper, zeros_feat, ones_edge, cudnn_ctx=cudnn_ctx)
        n_items = Fdim
        base_feat_for = ones_feat if mode == "deletion" else zeros_feat
        fixed_edge = ones_edge
    else:
        # baseline for insertion: remove all incident edges (but keep features)
        if Edim <= 0:
            return pd.DataFrame(), {"pred_full": pred_full, "pred_base": float("nan")}
        pred_base = _di_predict(wrapper, ones_feat, zeros_edge, cudnn_ctx=cudnn_ctx)
        n_items = Edim
        base_edge_for = ones_edge if mode == "deletion" else zeros_edge
        fixed_feat = ones_feat

    ks = _di_make_k_list(n_items=n_items, k_max=k_max, k_step=k_step)

    rows = []
    for st in strategies:
        st = str(st)
        reps = int(random_repeats) if st.lower() == "random" else 1
        for r in range(reps):
            rng = np.random.RandomState(int(seed) + 1337 * (hash(st) % 1000) + r)

            if kind == "feature":
                order = _di_get_order_feature(wrapper, feat_gate, st, rng)
            else:
                order = _di_get_order_edge(wrapper, edge_gate, st, rng)

            for k in ks:
                if kind == "feature":
                    gfeat = base_feat_for.clone()
                    if k > 0:
                        idx = torch.tensor(order[:k], device=wrapper.device, dtype=torch.long)
                        if mode == "deletion":
                            gfeat[idx] = 0.0
                        else:  # insertion
                            gfeat[idx] = 1.0
                    pred = _di_predict(wrapper, gfeat, fixed_edge, cudnn_ctx=cudnn_ctx)
                else:
                    gedge = base_edge_for.clone()
                    if k > 0:
                        idx = torch.tensor(order[:k], device=wrapper.device, dtype=torch.long)
                        if mode == "deletion":
                            gedge[idx] = 0.0
                            ...
                            gedge[idx] = 1.0
                    pred = _di_predict(wrapper, fixed_feat, gedge, cudnn_ctx=cudnn_ctx)

                # score drop / recovery
                delta_drop = float(pred_full - pred)  # deletionで + になりやすい
                # insertionの回復を0..1で比較できるように正規化
                denom = float(max(1e-12, pred_full - pred_base))
                gain_norm = float((pred - pred_base) / denom)  # insertionで 0->1 が理想
                ratio_full = float(pred / max(1e-12, pred_full))
                ...
                rows.append({
                    ...
                    "replicate": int(r),
                    "k": int(k),
                    "k_frac": float(k / max(1, n_items)),
                    "pred": float(pred),
                    "pred_full": float(pred_full),
                    "pred_base": float(pred_base),
                    "delta_drop(full-pred)": float(delta_drop),
                    "ratio(pred/full)": float(ratio_full),
                    "gain_norm((pred-base)/(full-base))": float(gain_norm),
                    "n_items": int(n_items),
                })

    df = pd.DataFrame(rows)
    meta = {"pred_full": float(pred_full), "pred_base": float(pred_base), "n_items": int(n_items)}
    return df, meta

def _di_plot_curves(df_curves: pd.DataFrame, out_prefix: str, title_prefix: str):
    """
    出力:
      - deletion: k vs delta_drop (mean±std)
      - insertion: k vs gain_norm (mean±std)
    """
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    for (kind, mode), sub in df_curves.groupby(["kind", "mode"], dropna=False):
        ...
        plt.figure(figsize=(10, 6))
        for st, gg in g.groupby("strategy"):
            gg = gg.sort_values("k")
            x = gg["k"].values
            y = gg["y_mean"].values
            ys = gg["y_std"].fillna(0.0).values
            plt.plot(x, y, marker="o", linewidth=2, label=str(st))
            if np.any(ys > 0):
                plt.fill_between(x, y - ys, y + ys, alpha=0.2)

        plt.xlabel("k (top-k removed/added)")
        if mode == "deletion":
            plt.ylabel("Score drop  Δ = pred_full - pred_k")
            plt.title(f"{title_prefix} | {kind} | Deletion curve")
        else:
            plt.ylabel("Normalized recovery  (pred_k - base)/(full - base)")
            plt.title(f"{title_prefix} | {kind} | Insertion curve")

        plt.grid(True, linestyle="--", linewidth=0.5)
        plt.legend(loc="best")
        plt.tight_layout()

        fpath = f"{out_prefix}_{kind}_{mode}.png"
...
def di_evaluate_and_log(
    ...
    target_node_idx: int,
    explain_pos: int,
    strategies_feat=("xai", "random", "reverse"),
    strategies_edge=("xai", "random", "degree", "reverse"),
    k_max=100,
    k_step=5,
    random_repeats=20,
    seed=42,
    mlflow_log=False,
    artifact_path="xai/di",
    cudnn_ctx=None,
):
    """
    1) feature/edge それぞれで deletion/insertion curves を作る
    2) CSV + plots + AUC(json) を保存（mlflow_log=Trueならartifactへ）
    """
    all_df = []

    # feature
    df_f_del, meta_f_del = _di_eval_kind_curves(
        wrapper, "feature", feat_gate, edge_gate, strategies_feat, "deletion",
        k_max, k_step, random_repeats, seed, cudnn_ctx=cudnn_ctx,
    )
    df_f_ins, meta_f_ins = _di_eval_kind_curves(
        wrapper, "feature", feat_gate, edge_gate, strategies_feat, "insertion",
        k_max, k_step, random_repeats, seed, cudnn_ctx=cudnn_ctx,
    )
    all_df += [df_f_del, df_f_ins]

    # edge (Edim==0なら空)
    df_e_del, meta_e_del = _di_eval_kind_curves(
        wrapper, "edge", feat_gate, edge_gate, strategies_edge, "deletion",
        k_max, k_step, random_repeats, seed, cudnn_ctx=cudnn_ctx,
    )
    df_e_ins, meta_e_ins = _di_eval_kind_curves(
        wrapper, "edge", feat_gate, edge_gate, strategies_edge, "insertion",
        k_max, k_step, random_repeats, seed, cudnn_ctx=cudnn_ctx,
    )
    all_df += [df_e_del, df_e_ins]

    df_all = pd.concat([d for d in all_df if d is not None and not d.empty], ignore_index=True) if any(
        d is not None and not d.empty for d in all_df
    ) else pd.DataFrame()

    if df_all.empty:
        return pd.DataFrame(), {}

    df_all.insert(0, "target_node", int(target_node_idx))
    df_all.insert(1, "explain_pos", int(explain_pos))
    df_all.insert(2, "tag", str(tag))

    # ---- AUC summary (per strategy, averaged over replicates) ----
    auc_rows = []
    for (kind, mode, st), sub in df_all.groupby(["kind", "mode", "strategy"], dropna=False):
        # compute AUC per replicate then mean±std
        aucs = []
        for r, s2 in sub.groupby("replicate"):
            s2 = s2.sort_values("k")
            x = s2["k_frac"].values
            if mode == "deletion":
                y = s2["ratio(pred/full)"].values  # lower faster drop => smaller AUC
            else:
                y = s2["gain_norm((pred-base)/(full-base))"].values  # higher faster recovery => larger AUC
            aucs.append(_di_auc(x, y))
    ...
    # ---- Save CSV + plots + json ----
    csv_path = f"di_curves_{tag}_node_{int(target_node_idx)}_pos_{int(explain_pos)}.csv"
    df_all.to_csv(csv_path, index=False, float_format="%.8e")

    auc_csv = f"di_auc_{tag}_node_{int(target_node_idx)}_pos_{int(explain_pos)}.csv"
    df_auc.to_csv(auc_csv, index=False, float_format="%.8e")

    title_prefix = f"node={int(target_node_idx)} pos={int(explain_pos)} ({tag})"
    out_prefix = f"di_plot_{tag}_node_{int(target_node_idx)}_pos_{int(explain_pos)}"
    plot_paths = _di_plot_curves(df_all, out_prefix=out_prefix, title_prefix=title_prefix)
    ...
    summary = {
        ...
        "auc_table": df_auc.to_dict(orient="records"),
    }

    import json
    js_path = f"di_summary_{tag}_node_{int(target_node_idx)}_pos_{int(explain_pos)}.json"
    ...
    if mlflow_log:
        try:
            ...
            mlflow.log_artifact(auc_csv, artifact_path=artifact_path)
            mlflow.log_artifact(js_path, artifact_path=artifact_path)
            for p in plot_paths:
                if p and os.path.exists(p):
                    mlflow.log_artifact(p, artifact_path=artifact_path)
        except Exception as e:
            print(f"⚠️ DI MLflow log failed: {e}")

    # cleanup local files (optional)
    for p in [csv_path, auc_csv, js_path] + list(plot_paths):
        try:
            if p and os.path.exists(p):
                os.remove(p)
        except Exception:
...
def maskopt_e2e_explain(
    ...
    # maskopt_e2e_explain の引数に追加（末尾付近でOK）
    di_eval=True,
    di_k_max=100,
    di_k_step=5,
    di_random_repeats=20,
    di_seed=42,
    di_strategies_feat=("xai","random","reverse"),
    di_strategies_edge=("xai","random","degree","reverse"),
    di_artifact_path="xai/di",
):
    ...
    if mlflow_log:
        ...
        # ===================== DI (Deletion/Insertion) Eval =====================
    di_summary = {}
    if bool(di_eval):
        try:
            _df_di, di_summary = di_evaluate_and_log(
                wrapper=wrapper,
                feat_gate=feat_gate,
                edge_gate=edge_gate,
                tag=str(tag),
                ...
                explain_pos=int(explain_pos),
                strategies_feat=tuple(di_strategies_feat),
                strategies_edge=tuple(di_strategies_edge),
                k_max=int(di_k_max),
                k_step=int(di_k_step),
                random_repeats=int(di_random_repeats),
                seed=int(di_seed),
                mlflow_log=bool(mlflow_log),
                artifact_path=str(di_artifact_path),
                cudnn_ctx=cudnn_ctx,   # 既に作ってる cudnn_ctx を流用
            )
            # metaにAUC等を埋め込む（論文の表に使える）
            meta["di"] = di_summary
        except Exception as e:
            print(f"⚠️ DI eval failed: {e}")
