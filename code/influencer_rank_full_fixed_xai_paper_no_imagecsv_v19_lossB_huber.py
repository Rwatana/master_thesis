        ...
        if mode_run != "infer":
            ...
            for epoch in range(params["EPOCHS"]):
                ...
                for batch in dataloader:
                    ...
                    def huber_loss(x, delta: float = 0.1):
                        # x: tensor
                        absx = x.abs()
                    ...
                    # ✅ pointwise は Huber（外れ値に強い）
                    delta = float(params.get("HUBER_DELTA", 0.1))
                    loss_point = huber_loss(log_pred - log_target, delta=delta).mean()
