...
def prepare_graph_data():
    ...
    # --- 論文に倣ったノイズ除去（プルーニング） ---
    print("Pruning auxiliary nodes with a single edge...")
    all_connections = pd.concat([
        df_hashtags[['username', 'hashtag']].rename(columns={'hashtag': 'target'}),
        df_mentions[['username', 'mention']].rename(columns={'mention': 'target'})
    ])
    source_counts = all_connections['username'].value_counts()
    target_counts = all_connections['target'].value_counts()
    all_counts = source_counts.add(target_counts, fill_value=0)
    
    nodes_to_remove = set(all_counts[
        (~all_counts.index.isin(influencer_set)) & (all_counts <= 1)
    ].index)
    
    all_hashtags = set(df_hashtags['hashtag'].astype(str)) - nodes_to_remove
    all_mentions = set(df_mentions['mention'].astype(str)) - nodes_to_remove
    # --- 完了 ---

    all_nodes = sorted(list(influencer_set | all_hashtags | all_mentions))
    node_to_idx = {node: i for i, node in enumerate(all_nodes)}
    num_nodes = len(all_nodes)
    print(f"Total unique nodes after pruning: {num_nodes}")
