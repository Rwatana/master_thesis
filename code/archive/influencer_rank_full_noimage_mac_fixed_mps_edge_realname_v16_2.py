...
class E2EMaskOptWrapper(nn.Module):
    ...
    def _prepare_explain_graph(self):
        # 【修正】 self.graphs -> self.input_graphs
        data_full = self.input_graphs[self.explain_pos]
        
        # data_full を device に転送 (MPS/CUDA対応)
        data_full = data_full.to(self.device)
        
        x_full = data_full.x
        edge_index_full = data_full.edge_index
        num_nodes = int(data_full.num_nodes)

        # 【修正】 self.target_node -> self.target_global
        target_global = int(self.target_global)
        ...
        if inc_mask.sum().item() == 0:
            ...
            idx_map = torch.full((num_nodes,), -1, dtype=torch.long, device=self.device)
            idx_map[subset] = torch.arange(subset.numel(), device=self.device)

            ei = edge_index_full[:, inc_mask]
            ei_sub = idx_map[ei]
            valid = (ei_sub[0] >= 0) & (ei_sub[1] >= 0)
            ei_sub = ei_sub[:, valid]
...
def maskopt_e2e_explain(
    ...
    save_history_prefix=None,
    save_history_every=1,
    mask_threshold=0.5,
):
    ...
    def _edge_label_and_endpoints(edge_k: int, ei_exp: torch.Tensor, target_idx: int):
        # ei_exp: [2, E] edge_index in the explained (sub)graph
        s = int(ei_exp[0, edge_k].item())
        d = int(ei_exp[1, edge_k].item())
        sname = _node_name(s)
        dname = _node_name(d)
        t = int(target_idx)
        if s == t:
            ...
            neigh = dname
        label = f"{sname}--{dname} (e{int(edge_k)})"
        return label, sname, dname, neigh
    ...
    for ep in range(1, epochs + 1):
        ...
        if (save_history_every is not None) and (save_history_every > 0) and (ep % int(save_history_every) == 0):
            loss_history.append(
                {
                    "epoch": int(ep),
            ...
            feat_mask = (feat_gate < float(mask_threshold)).to(torch.uint8)
            feat_mask_history.append(feat_mask.detach().cpu().numpy())
            if edge_gate is not None and edge_gate.numel() > 0:
                edge_mask = (edge_gate < float(mask_threshold)).to(torch.uint8)
                edge_mask_history.append(edge_mask.detach().cpu().numpy())
    ...
    # ei_exp は [2, E] のテンソル (ローカルID)
    src_local_all = wrapper.ei_exp[0, :].cpu().numpy()
    dst_local_all = wrapper.ei_exp[1, :].cpu().numpy()
    local2global = wrapper.local2global  # [N_sub] テンソル

    for k in edge_iter:
        imp = float(edge_np[k])
        
        # 1. ローカルIDを取得
        u_local = int(src_local_all[k])
        v_local = int(dst_local_all[k])
        
        # 2. グローバルIDに変換
        u_global = int(local2global[u_local].item())
        v_global = int(local2global[v_local].item())
        
        # 3. 名前を取得 (変数名を sname, dname に統一)
        sname = _node_name(u_global)
        dname = _node_name(v_global)
        
        # 4. ターゲットとの関係を確認
        tgt_global = int(target_node_idx)
        if u_global == tgt_global:
            neighbor = dname
        elif v_global == tgt_global:
            neighbor = sname
        else:
            neighbor = dname 
            
        label = f"{sname}--{dname} (e{int(k)})"

        row = {
            "Type": "Edge",
            "Name": label,
            "EdgeIdx": int(k),
            "Src": sname,       # 【ここを src_name ではなく sname にする】
            "Dst": dname,       # 【ここを dst_name ではなく dname にする】
            "Neighbor": neighbor,
            "Importance": imp,
            "OrigScore": orig_score,
        }
    ...
    def _print_df_summary(df, name: str):
        ...
        # show top-10 by importance and by abs impact (orig-ablated)
        if "Score_Impact(orig-ablated)" in df.columns:
            top_imp = df.sort_values("Importance", ascending=False).head(10)
            top_abs = df.assign(_abs=df["Score_Impact(orig-ablated)"].abs()).sort_values("_abs", ascending=False).head(10)
            print(f"  Top-10 by Importance:")
            print(top_imp[["Type","Name","Importance","Score_Impact(orig-ablated)"]].to_string(index=False))
            print(f"  Top-10 by |Score_Impact(orig-ablated)|:")
            print(top_abs[["Type","Name","Importance","Score_Impact(orig-ablated)"]].to_string(index=False))
    ...
    if save_history_dir is not None:
        ...
        prefix = save_history_prefix if save_history_prefix is not None else str(tag)
        loss_csv = os.path.join(save_history_dir, f"maskopt_loss_history_{prefix}.csv")
        mask_npz = os.path.join(save_history_dir, f"maskopt_mask_history_{prefix}.npz")
        feat_names_csv = os.path.join(save_history_dir, f"maskopt_feature_names_{prefix}.csv")
        edge_names_csv = os.path.join(save_history_dir, f"maskopt_edge_names_{prefix}.csv")
        meta_json = os.path.join(save_history_dir, f"maskopt_meta_{prefix}.json")

        if len(loss_history) > 0:
            df_loss = pd.DataFrame(loss_history)
            df_loss["feat_masked_frac"] = np.mean(np.stack(feat_mask_history, axis=0), axis=1)
            if len(edge_mask_history) > 0:
                df_loss["edge_masked_frac"] = np.mean(np.stack(edge_mask_history, axis=0), axis=1)
            else:
                df_loss["edge_masked_frac"] = 0.0
            df_loss.to_csv(loss_csv, index=False, float_format="%.8e")

        feat_name_rows = [{"feat_idx": int(i), "name": str(feature_names[i])} for i in range(len(feature_names))]
        pd.DataFrame(feat_name_rows).to_csv(feat_names_csv, index=False)

        edge_name_rows = []
        if wrapper.ei_exp is not None and wrapper.ei_exp.numel() > 0:
            src_local_all = wrapper.ei_exp[0, :].detach().cpu().numpy()
            dst_local_all = wrapper.ei_exp[1, :].detach().cpu().numpy()
            local2global = wrapper.local2global.detach().cpu().numpy()
            for k in range(int(wrapper.ei_exp.shape[1])):
                u_local = int(src_local_all[k])
                v_local = int(dst_local_all[k])
                u_global = int(local2global[u_local])
                v_global = int(local2global[v_local])
                sname = _node_name(u_global)
                dname = _node_name(v_global)
                label = f"{sname}--{dname} (e{int(k)})"
                edge_name_rows.append(
                    {"edge_idx": int(k), "label": label, "src": sname, "dst": dname, "src_idx": u_global, "dst_idx": v_global}
                )
        pd.DataFrame(edge_name_rows).to_csv(edge_names_csv, index=False)

        feat_mask_arr = np.stack(feat_mask_history, axis=0) if len(feat_mask_history) > 0 else np.zeros((0, Fdim), dtype=np.uint8)
        edge_mask_arr = np.stack(edge_mask_history, axis=0) if len(edge_mask_history) > 0 else np.zeros((0, Edim), dtype=np.uint8)
        np.savez_compressed(
            mask_npz,
            feat_mask=feat_mask_arr,
        ...
        meta_out.update(
            {
                "mask_threshold": float(mask_threshold),
                "save_history_every": int(save_history_every),
                "loss_csv": loss_csv,
                "mask_npz": mask_npz,
                "feature_names_csv": feat_names_csv,
                "edge_names_csv": edge_names_csv,
            }
        )
        with open(meta_json, "w", encoding="utf-8") as f:
            json.dump(meta_out, f, indent=2, ensure_ascii=True)
        meta["history_files"] = {
            "loss_csv": loss_csv,
            "mask_npz": mask_npz,
            "feature_names_csv": feat_names_csv,
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        if params.get("XAI_ONLY", False):
            ...
            if local_pth_path and os.path.exists(local_pth_path):
                ...
                mlflow.log_param("loaded_local_model_path", local_pth_path)
            
            # 指定がなければMLflowから探す（既存ロジック）
            else:
                tracking_uri = params.get("MLFLOW_TRACKING_URI", None)
                exp_name = params.get("MLFLOW_EXPERIMENT_NAME", "InfluencerRankSweep")
                artifact_name = params.get("MLFLOW_MODEL_ARTIFACT", "model_state_dict.pth")
                desired_run_id = params.get("MLFLOW_MODEL_RUN_ID", None)

                print(f"[XAI_ONLY] Attempting to download model from MLflow Run ID: {desired_run_id}")
                loaded_model_run_id, loaded_model_local_path = mlflow_download_model_artifact(
                    tracking_uri=tracking_uri,
                    experiment_name=exp_name,
                    artifact_basename=artifact_name,
                    preferred_subdir="model",
                    run_id=desired_run_id,
                    dst_dir="downloaded_models",
                )
                if loaded_model_run_id is None:
                    raise RuntimeError(
                        f"XAI_ONLY requested but no model found via MLflow (and no local --model_path provided).\n"
                        f"experiment={exp_name} artifact={artifact_name} run_id={desired_run_id}"
                    )

                state = torch.load(loaded_model_local_path, map_location=device)
                model.load_state_dict(state)
                model.eval()
                print(f"[MLflow] Loaded trained model: run_id={loaded_model_run_id} -> {loaded_model_local_path}")
                mlflow.log_param("loaded_model_run_id", loaded_model_run_id)
            ...
            print(f"✅ Model saved to MLflow artifact: model/{model_filename}")
        ...
        try:
            ...
            attn_w = None
            global2local = {int(g): i for i, g in enumerate(influencer_indices)}
            row = global2local.get(int(target_node_global_idx), None)
            if row is not None and row < len(attention_matrix):
            ...
            for explain_pos in positions_to_explain:
                ...
                df_feat, df_edge, meta = maskopt_e2e_explain(
                    ...
                    budget_edge=20,
                    budget_weight=1.0,
                    impact_reference="unmask",
                    use_contrastive=False,
                    tag=tag,
                    mlflow_log=True,
                    save_history_dir=params.get("MASKOPT_VIZ_DIR"),
                    save_history_prefix=f"node_{target_node_global_idx}_{tag}_{run_name}",
                    save_history_every=int(params.get("MASKOPT_SAVE_EVERY", 1)),
                    mask_threshold=float(params.get("MASKOPT_MASK_THRESHOLD", 0.5)),
                )

                print(f"{tag} orig={float(meta.get('orig_pred')):.6f} unmasked={float(meta.get('pred_unmasked')):.6f} masked={float(meta.get('pred_masked')):.6f} best={float(meta.get('best_pred')):.6f}")
                ...
                if params.get("PLOT_SCALE_0_1", False):
                    # helper defined locally or assumed global
                    def _scatter_absimpact_norm(df, title, fname):
                        if df is None or len(df) == 0: return
                        if "Score_Impact(orig-ablated)" not in df.columns: return
                        
                        feat_abs = df_feat["Score_Impact(orig-ablated)"].abs() if df_feat is not None else pd.Series([0])
                        edge_abs = df_edge["Score_Impact(orig-ablated)"].abs() if df_edge is not None else pd.Series([0])
                        denom = float(max(feat_abs.max(), edge_abs.max(), 1e-12))
                        
                        dfp = df.copy()
                        dfp["_y"] = dfp["Score_Impact(orig-ablated)"].abs() / denom
                        dfp["_x"] = dfp["Importance"].clip(lower=0.0, upper=1.0)
                        plt.figure(figsize=(6, 4))
                        plt.scatter(dfp["_x"], dfp["_y"], s=12, alpha=0.7)
                        plt.xlabel("Importance (0..1)")
                        plt.ylabel("|Score impact| normalized (0..1)")
                        plt.title(title)
                        plt.tight_layout()
                        mlflow.log_figure(plt.gcf(), _sanitize_mlflow_name(fname))
                        plt.close()

                    if df_feat is not None:
                        _scatter_absimpact_norm(df_feat, f"MaskOpt: Features importance vs |impact| (norm) {tag}", f"node_{target_node_global_idx}_{tag}_scatter_features_absimpact_norm.png")
                    if df_edge is not None:
                        _scatter_absimpact_norm(df_edge, f"MaskOpt: Incident edges importance vs |impact| (norm) {tag}", f"node_{target_node_global_idx}_{tag}_scatter_edges_absimpact_norm.png")
...
def main():
    # --- CLI Args Parsing ---
    parser = argparse.ArgumentParser(description="InfluencerRank Training & XAI Script")
    parser.add_argument("--device", type=str, default="auto", help="auto|cpu|mps|0")
    parser.add_argument("--visible", type=str, default=None, help="CUDA_VISIBLE_DEVICES")
    
    # XAI & MLflow args
    parser.add_argument("--xai_only", action="store_true", help="Skip training, load model and explain")
    parser.add_argument("--model_path", type=str, default=None, help="Path to local .pth model file (skips MLflow download)")
    
    parser.add_argument("--mlflow_tracking_uri", type=str, default=None)
    parser.add_argument("--mlflow_experiment_name", type=str, default="InfluencerRankSweep")
    parser.add_argument("--mlflow_model_run_id", type=str, default=None, help="Run ID to load model from (if model_path is None)")
    parser.add_argument("--mlflow_model_artifact", type=str, default="model_state_dict.pth")
    parser.add_argument("--plot_scale_0_1", action="store_true", help="Enable normalized impact scatter plots")
    parser.add_argument("--maskopt_viz_dir", type=str, default="xai_viz", help="Directory to save MaskOpt loss/mask history")
    parser.add_argument("--maskopt_mask_threshold", type=float, default=0.5, help="Mask threshold for feature/edge gate (< threshold = masked)")
    parser.add_argument("--maskopt_save_every", type=int, default=1, help="Save MaskOpt history every N epochs")
    ...
    experiment_name, experiment_id = setup_mlflow_experiment(
        experiment_base_name=os.environ.get("MLFLOW_EXPERIMENT_NAME", args.mlflow_experiment_name),
        tracking_uri=os.environ.get("MLFLOW_TRACKING_URI", args.mlflow_tracking_uri),
    ...
    base_params = {
        ...
        # CLI injection
        "XAI_ONLY": args.xai_only,
        "MODEL_PATH": args.model_path,  # ここを追加
        "PLOT_SCALE_0_1": args.plot_scale_0_1,
        "MLFLOW_TRACKING_URI": args.mlflow_tracking_uri,
        "MLFLOW_EXPERIMENT_NAME": args.mlflow_experiment_name,
        "MLFLOW_MODEL_RUN_ID": args.mlflow_model_run_id,
        "MLFLOW_MODEL_ARTIFACT": args.mlflow_model_artifact,
        "MASKOPT_VIZ_DIR": args.maskopt_viz_dir,
        "MASKOPT_MASK_THRESHOLD": args.maskopt_mask_threshold,
        "MASKOPT_SAVE_EVERY": args.maskopt_save_every,
    }

    # Parameter sweep logic (list of dicts)
    params_list = []
    ...
    overrides_list = list(params_list) if len(params_list) > 0 else [{}] 
