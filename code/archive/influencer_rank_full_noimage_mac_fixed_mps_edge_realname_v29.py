...
class E2EMaskOptWrapper(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.target_node = int(target_node_idx)  # alias for backward-compat
...
def maskopt_e2e_explain(
    ...
    def _edge_label_and_endpoints(edge_k, ei, target_local, node_to_idx, local2global=None):
        """Return oriented (target -> neighbor) endpoints for an edge in the explanation graph.

        Keeps ONLY edges incident to the target node. If the edge is not incident (can happen if indices
        mismatch), returns (None, None, None, None) so the caller can skip.
        """
        s_l = int(ei[0, edge_k])
        d_l = int(ei[1, edge_k])

        if local2global is not None:
            s_g = int(local2global[s_l])
            d_g = int(local2global[d_l])
            t_g = int(local2global[int(target_local)])
        else:
            s_g, d_g, t_g = int(s_l), int(d_l), int(target_local)
        ...
        t_name = node_name(node_to_idx, t_g)
        n_name = node_name(node_to_idx, n_g)
        label = f"{t_name}--{n_name}"
        return label, t_name, n_name, n_name
    ...
    for k in edge_iter:
        ...
        try:
            ...
            continue
        if (src_local != int(wrapper.target_local)) and (dst_local != int(wrapper.target_local)):
            continue
        imp = float(edge_np[k])
        label, src_name, dst_name, neighbor = _edge_label_and_endpoints(int(k), wrapper.ei_exp, wrapper.target_local, node_to_idx, local2global=wrapper.local2global)
        if label is None:
            continue
        row = {
            "Type": "Edge",
...
def run_experiment(params, graphs_data, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        # 1) Prefer local .pth if provided
        model_pth_path = (params.get("MODEL_PTH_PATH", "") or "").strip()
        if model_pth_path:
            if os.path.exists(model_pth_path):
                try:
                    ...
                    model.to(device)
                    model.eval()
                    loaded_model_run_id = "<local_pth>"
                    loaded_model_local_path = model_pth_path
                    model_loaded = True
                    print(f"[Model] ✅ Loaded model from .pth: {model_pth_path}")
                    mlflow.log_param("loaded_model_source", "local_pth")
                    mlflow.log_param("loaded_model_local_path", model_pth_path)
                except Exception as e:
                    print(f"[Model] ⚠️ Failed to load MODEL_PTH_PATH='{model_pth_path}': {e} (will train)")
            else:
                print(f"[Model] ⚠️ MODEL_PTH_PATH not found: {model_pth_path} (will train)")

        # 2) Load from MLflow run artifact if requested (XAI-only / inference-only)
        if (not model_loaded) and (params.get("MLFLOW_MODEL_RUN_ID") is not None) and str(params.get("MLFLOW_MODEL_RUN_ID")).strip():
            ...
            desired_run_id = str(params.get("MLFLOW_MODEL_RUN_ID")).strip()
            ...
            if loaded_model_local_path and os.path.exists(loaded_model_local_path):
                try:
                    ...
                    model.eval()
                    model_loaded = True
                    print(f"[Model] ✅ Loaded model from MLflow: run_id={loaded_model_run_id} -> {loaded_model_local_path}")
                    mlflow.log_param("loaded_model_source", "mlflow_artifact")
                    mlflow.log_param("loaded_model_run_id", loaded_model_run_id)
                    mlflow.log_param("loaded_model_local_path", loaded_model_local_path)
                except Exception as e:
                    print(f"[Model] ⚠️ Failed to load MLflow model artifact: {e} (will train)")

        # 3) Decide whether to train
        inference_only = bool(params.get("INFERENCE_ONLY", False)) or bool(params.get("XAI_ONLY", False)) or bool(params.get("SKIP_TRAINING", False))
        if model_pth_path:
            # If the user gave a .pth, default to inference-only
            inference_only = True
        if inference_only and (not model_loaded):
            print("[Model] ⚠️ Inference-only requested but no model could be loaded; training a new model.")
            inference_only = False

        epochs_to_run = 0 if inference_only else int(params["EPOCHS"])
        # Always define `last_epoch` so downstream MLflow logging doesn't crash
        ...
        if epochs_to_run > 0:
            ...
            for epoch in range(epochs_to_run):
                optimizer.zero_grad()
                y_pred = model(graphs_data['node_features_seq'], graphs_data['adjacency_matrices'])
                loss = loss_fn(y_pred, graphs_data['y_true'])
                loss.backward()
                optimizer.step()
                if (epoch + 1) % 10 == 0:
                    print(f"Epoch {epoch+1}/{epochs_to_run} Loss: {loss.item():.4f}")

            # Training finished
            last_epoch = epochs_to_run
            # Save trained model to MLflow artifacts
            try:
                state_dict_path = os.path.join(os.getcwd(), "model_state_dict.pth")
                torch.save(model.state_dict(), state_dict_path)
                mlflow.log_artifact(state_dict_path, artifact_path="model")
                print(f"[MLflow] ✅ Saved trained model artifact: run_id={run_id} -> model/model_state_dict.pth")
                print(f"[MLflow]    local_path={state_dict_path}")
                mlflow.log_param("trained_model_run_id", run_id)
                mlflow.log_param("trained_model_local_path", state_dict_path)
            except Exception as e:
                print(f"[MLflow] ⚠️ Failed to save trained model artifact: {e}")
        else:
            print("[Model] Skipping training (inference-only).")
        print("\nStarting Inference...")
        model.eval()
        ...
        print(f"\n🎯 Selected Hub Influencer: {node_name(node_to_idx, target_node_global_idx)} (global_idx={target_node_global_idx}) with {int(max_degree)} incident edges.")
