...
class E2EMaskOptWrapper(nn.Module):
    ...
    def _apply_feature_gate(self, x, feat_gate):
        ...
        x2[t:t+1, :] = x2[t:t+1, :] * g + base * (1.0 - g)
        return x2
...
def maskopt_e2e_explain(
    ...
    edge_rows = []
    target_label = _node_name(target_node)
    for k in edge_iter:
        imp = float(edge_np[k])
        label, src_name, dst_name, neighbor = _edge_label_and_endpoints(edge_pairs, edge_names, k)
        row = {
            "Type": "Edge",
            "Name": f"{target_label}--{neighbor} (e{int(k)})",
            "EdgeIdx": int(k),
            "Src": src_name,
            "Dst": dst_name,
            "Neighbor": neighbor,
