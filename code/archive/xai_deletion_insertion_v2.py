...
from influencer_rank.mlflow_utils import maybe_download_ckpt_from_mlflow, load_model_from_ckpt
...
def _load_ckpt_state_dict(ckpt_path: str, map_location: str = "cpu") -> Dict[str, "np.ndarray"]:
    ...
    if isinstance(obj, dict):
        # sometimes the state dict is the whole dict already
        if any(k.endswith(".weight") for k in obj.keys()):
            return obj
    raise ValueError(f"Unexpected ckpt format: {type(obj)} keys={list(obj.keys())[:10] if isinstance(obj, dict) else 'n/a'}")
...
def main():
    ...
    monthly_graphs = graphs_data["monthly_graphs"]
    feature_names = graphs_data["feature_names"]
    node_to_idx = graphs_data["node_to_idx"]
    ...
    device = args.device
    model = load_model_from_ckpt(model, ckpt_path, device=device)
    model.eval()
