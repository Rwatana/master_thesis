    ...
    try:
        ...
        def _log_rank_scatter(df, impact_col, fname, title):
            ...
            if spearman == spearman:
                mlflow.log_metric(f"maskopt_spearman_importance_vs_absimpact_{impact_col}", float(spearman))
            fig = plt.figure()
            plt.scatter(tmp["Rank_Importance"], tmp["Rank_ImpactAbs"], s=8)
            plt.xlabel("Rank (Importance) [1=highest]")
            plt.ylabel(f"Rank (|{impact_col}|) [1=highest]")
            plt.title(title)
            plt.tight_layout()
            mlflow.log_figure(fig, fname)
            plt.close(fig)

        _log_rank_scatter(df_feat, ref_col, f"{tag}_feat_rank_scatter_zero.png",
                          f"Features: rank(Importance) vs rank(|Score impact|) [{ref_col}/zero]")
        _log_rank_scatter(df_edge, ref_col, f"{tag}_edge_rank_scatter_zero.png",
