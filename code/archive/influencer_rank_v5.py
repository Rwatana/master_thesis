...
MODEL_SAVE_PATH = f'influencer_rank_model_{time.strftime("%Y%m%d")}_rich_features_2017_3rd_v5_cpu.pth'
...
def prepare_graph_data(end_date, num_months=12, metric_numerator='likes', metric_denominator='posts'):
    ...
    print(f"Total raw feature dimension: {FEATURE_DIM}") # ✅ "raw" を追加
...
class InfluencerRankModel(nn.Module):
    def __init__(self, feature_dim, gcn_dim, rnn_dim, num_gcn_layers=2, dropout_prob=0.5, projection_dim=128):
        """
        feature_dim: 元の生の特徴量の次元数
        projection_dim: GCNに入力する前に射影する次元数 (デフォルト128)
        gcn_dim: GCNの各隠れ層の次元数
        """
        ...
        # 1. 射影層 (Raw Features -> 128)
        self.projection_layer = nn.Sequential(
            Linear(feature_dim, projection_dim),
            ReLU()
        )
        
        # 2. GCN (128 -> gcn_dim * num_gcn_layers)
        # GCNEncoderの入力次元を projection_dim に変更
        self.gcn_encoder = GCNEncoder(projection_dim, gcn_dim, num_gcn_layers)
        
        # 3. RNN (gcn_dim * num_gcn_layers -> rnn_dim)
...
def train_and_save_model():
    ...
    PROJECTION_DIM = 128 # ✅ GCNへの入力次元数を定義
    ...
    # ✅ モデル初期化時に projection_dim=128 を渡す
    ...
    if not END_TO_END_TRAINING:
        ...
        model.projection_layer.eval() # ✅ 射影層も評価モード
        ...
        with torch.no_grad():
            ...
            for i, g in enumerate(tqdm(monthly_graphs, desc="Projection + GCN Encoding")):
                ...
                projected_x = model.projection_layer(g.x) # ✅ 射影
                ...
                gcn_out = model.gcn_encoder(projected_x, g.edge_index) # ✅ GCN
        ...
        print(f"[3] Target Embeddings (Batch, Seq, Feat): {debug_target_embeddings.shape}")
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for batch_indices, batch_true_scores in tqdm(dataloader, desc=f"Epoch {epoch+1}/{NUM_EPOCHS}"):
                ...
                # ✅ model.forward を使って予測 (デバッグプリントはオフ)
...
def run_inference():
    ...
    with torch.no_grad():
        print("\n--- 🐛 DEBUG: Inference ---")
        # ✅ モデルのforwardメソッドを使って予測 (デバッグプリントを有効化)
        predicted_scores = model(inference_input_graphs, predict_indices, debug_print=True)
        print("--- 🐛 End Debug ---")
