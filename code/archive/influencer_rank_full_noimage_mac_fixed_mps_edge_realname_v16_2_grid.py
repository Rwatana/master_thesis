...
def maskopt_e2e_explain(
    ...
    for k in edge_iter:
        imp = float(edge_np[k])
        label, src_name, dst_name, neighbor = _edge_label_and_endpoints(int(k), wrapper.ei_exp, target_node_idx)
        row = {
            "Type": "Edge",
            "Name": label,
            "EdgeIdx": int(k),
...
def main():
    ...
    base_params = {
        "name_prefix": "XAI_Sweep",
        "note": "Optimizing XAI loss",
        
        # --- モデル構築に必要なパラメータ (これらが不足していました) ---
        "DROPOUT_PROB": 0.2,
        "GCN_DIM": 128,
        "RNN_DIM": 128,
        "NUM_GCN_LAYERS": 2,
        ...
        # --- ログ記録やデータローダーに必要なパラメータ ---
        "BATCH_SIZE": 50 * 64,
        "LIST_SIZE": 50,
        "EPOCHS": 150,     # 学習はしませんが辞書には必要
        "LR": 0.05,        # XAIの学習率として使用
    ...
    # 試したい条件をリストにする
    sweep_grid = {
        "fid_weight": [10.0, 100.0, 1000.0],       # 忠実度の重みを変えてみる
        "size_weight": [0.01, 0.05, 0.2],          # スパース性の重みを変えてみる
        "use_contrastive": [False, True],          # Contrastive Lossの有無
    }
