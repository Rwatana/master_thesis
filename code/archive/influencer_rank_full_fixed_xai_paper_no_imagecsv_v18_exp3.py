...
# ===================== Stability Test (Reproducibility) =====================

def set_global_seed(seed: int):
    """Best-effort determinism for MaskOpt initialization / optimizer path."""
    seed = int(seed)
    random.seed(seed)
    np.random.seed(seed)
    ...
    # cudnn determinism (MaskOpt already can disable cudnn during wrapper forward)
    try:
...
def _extract_topk_names(df: pd.DataFrame, k: int, score_col: str = "Importance", name_col: str = "Name"):
    if df is None or df.empty:
        return []
    d = df.copy()
    if score_col in d.columns:
        d = d.sort_values(score_col, ascending=False)
    return d[name_col].astype(str).head(int(k)).tolist()
...
def _kendall_topk(a, b, k: int):
    """
    Kendall tau over partial rankings.
    Missing items are assigned rank k+1.
    """
    k = int(k)
    a = list(a[:k])
    b = list(b[:k])
    # union (stable order)
    items = list(dict.fromkeys(a + b))
    if len(items) <= 1:
        return 0.0
    rank_a = {it: i + 1 for i, it in enumerate(a)}
    rank_b = {it: i + 1 for i, it in enumerate(b)}
    miss_rank = k + 1
    va = [rank_a.get(it, miss_rank) for it in items]
    vb = [rank_b.get(it, miss_rank) for it in items]
    tau, _ = kendalltau(va, vb)
    if tau is None or (isinstance(tau, float) and (np.isnan(tau) or np.isinf(tau))):
        return 0.0
    return float(tau)


def run_maskopt_stability_test(
    model,
    input_graphs,
    target_node_idx: int,
    positions: list[int],
    ...
    n_runs: int = 20,
    topk: int = 20,
    kind: str = "both",  # "feat" | "edge" | "both"
    base_seed: int = 1234,
    # MaskOpt hyperparams (keep consistent with your main XAI run by default)
    epochs: int = 300,
    lr: float = 0.05,
    fid_weight: float = 2000.0,
    coeffs=None,
    budget_feat: int | None = 10,
    budget_edge: int | None = 20,
    budget_weight: float = 1.0,
    use_subgraph: bool = True,
    num_hops: int = 1,
    edge_mask_scope: str = "incident",
    edge_grouping: str = "neighbor",
    impact_reference: str = "masked",
    # logging
    mlflow_log: bool = True,
    artifact_path: str = "xai/stability",
):
    """
    Reproducibility test:
      - run MaskOpt N times with different seeds / init
      - collect top-k Feature / Neighbor (edge-group) names by Importance (gate)
      - compute pairwise Jaccard / overlap@k / Kendall tau
      - output CSV + boxplots (month/pos comparison)
    """
    ...
    assert kind in ("feat", "edge", "both"), f"kind must be feat|edge|both, got {kind}"

    # Make sure positions are ints and unique (keep order)
    pos_list = []
    seen = set()
    for p in positions:
        p = int(p)
    ...
    run_rows = []       # per run (pos, seed, topk list strings)
    pair_rows = []      # pairwise metrics rows

    # For boxplot: store distributions by (pos, kind)
    dist = {}  # (pos, kind, metric) -> list[float]

    for pos in pos_list:
        # Run N times for same (target_node, pos)
        topk_feat_lists = []
        topk_edge_lists = []
        ...
        for r in range(int(n_runs)):
            ...
            tag = f"stab_pos_{pos}_seed_{seed}"
            try:
                df_feat, df_edge, meta = maskopt_e2e_explain(
                    ...
                    # IMPORTANT: stability test wants less noise in outputs / speed
                    mlflow_log=False,
                    log_diagnostics=False,
                    log_topk_each_epoch=False,
                    print_every=max(50, int(epochs)//3),
                    tag=tag,
                )
            except Exception as e:
                print(f"⚠️ [Stability] MaskOpt failed pos={pos} seed={seed}: {e}")
                df_feat, df_edge = pd.DataFrame(), pd.DataFrame()

            feat_top = _extract_topk_names(df_feat, topk) if kind in ("feat","both") else []
            edge_top = _extract_topk_names(df_edge, topk) if kind in ("edge","both") else []

            if kind in ("feat","both"):
                topk_feat_lists.append(feat_top)
            if kind in ("edge","both"):
                topk_edge_lists.append(edge_top)

            run_rows.append({
                "target_node": int(target_node_idx),
                "pos": int(pos),
                "month_label": f"{pos+1}月(pos={pos})",
                "run_index": int(r),
                "seed": int(seed),
                "topk": int(topk),
                "top_feat": " | ".join(feat_top),
                "top_edge": " | ".join(edge_top),
            })

        # ---- pairwise metrics (all pairs i<j) ----
        def _all_pairs(lists, kind_label: str):
            m = len(lists)
            for i in range(m):
                for j in range(i+1, m):
                    ...
                    b = lists[j]
                    jac = _jaccard(a, b)
                    ovk = _overlap_at_k(a, b, topk)
                    tau = _kendall_topk(a, b, topk)
                    ...
                    pair_rows.append({
                        ...
                        "i": int(i),
                        "j": int(j),
                        "seed_i": int(base_seed) + int(i),
                        "seed_j": int(base_seed) + int(j),
                        "topk": int(topk),
                        "jaccard": float(jac),
                        "overlap_at_k": float(ovk),
                        "kendall_tau": float(tau),
                    })

                    dist.setdefault((pos, kind_label, "overlap_at_k"), []).append(float(ovk))
                    dist.setdefault((pos, kind_label, "jaccard"), []).append(float(jac))
                    dist.setdefault((pos, kind_label, "kendall_tau"), []).append(float(tau))

        if kind in ("feat","both"):
            _all_pairs(topk_feat_lists, "Feature")
        if kind in ("edge","both"):
            _all_pairs(topk_edge_lists, "Neighbor")  # because edge_grouping=neighbor

    df_runs = pd.DataFrame(run_rows)
    df_pairs = pd.DataFrame(pair_rows)

    # ---- summary table (mean/std by month & kind) ----
    df_summary = pd.DataFrame()
    if not df_pairs.empty:
        df_summary = (
            df_pairs
            .groupby(["month_label", "kind"], as_index=False)
            .agg(
                n=("overlap_at_k", "count"),
                overlap_mean=("overlap_at_k", "mean"),
                overlap_std=("overlap_at_k", "std"),
    ...
    # ---- plots: boxplot month-wise ----
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    plot_files = []
    for kind_label in ["Feature", "Neighbor"]:
        if kind == "feat" and kind_label != "Feature":
            continue
        if kind == "edge" and kind_label != "Neighbor":
            continue

        for metric in ["overlap_at_k", "jaccard", "kendall_tau"]:
            # build data in pos_list order
            data = []
            labels = []
            for pos in pos_list:
                vals = dist.get((pos, kind_label, metric), [])
                if len(vals) == 0:
                    continue
                data.append(vals)
                labels.append(f"{pos+1}月")
            ...
            plt.ylim(-0.05, 1.05 if metric != "kendall_tau" else 1.05)
            plt.xlabel("Month (pos)")
            plt.ylabel(metric)
            plt.title(f"Stability ({kind_label}) : {metric}  (topk={topk}, runs={n_runs})")
            plt.tight_layout()
            out_png = f"stability_box_{kind_label}_{metric}_node_{int(target_node_idx)}_topk_{int(topk)}.png"
            plt.savefig(out_png, dpi=220)
            plt.close()
            plot_files.append(out_png)

    # ---- write CSVs ----
    runs_csv  = f"stability_runs_node_{int(target_node_idx)}_topk_{int(topk)}.csv"
    pairs_csv = f"stability_pairs_node_{int(target_node_idx)}_topk_{int(topk)}.csv"
    df_runs.to_csv(runs_csv, index=False)
    df_pairs.to_csv(pairs_csv, index=False)

    summ_csv = None
    if df_summary is not None and not df_summary.empty:
        summ_csv = f"stability_summary_node_{int(target_node_idx)}_topk_{int(topk)}.csv"
    ...
    if bool(mlflow_log):
        try:
            ...
            if _ml.active_run() is not None:
                _ml.log_artifact(runs_csv, artifact_path=artifact_path)
                _ml.log_artifact(pairs_csv, artifact_path=artifact_path)
                if summ_csv is not None:
                    _ml.log_artifact(summ_csv, artifact_path=artifact_path)
                for f in plot_files:
                    if os.path.exists(f):
                        _ml.log_artifact(f, artifact_path=artifact_path)
        except Exception as e:
            print(f"⚠️ [Stability] MLflow log failed: {e}")

    # cleanup local files
    for f in [runs_csv, pairs_csv, summ_csv] + plot_files:
    ...
    return df_runs, df_pairs, df_summary
...
def run_experiment(params, graphs_data, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        try:
            ...
            # ===================== Stability Test (Reproducibility) =====================
            if bool(params.get("stability_test", False)):
                def _parse_int_list(s: str, default=None):
                    if default is None:
                        default = []
                    if s is None:
                        return list(default)
                ...
                stab_positions = _parse_int_list(params.get("stability_positions", "0,1,2"), default=[0,1,2])
                stab_runs = int(params.get("stability_runs", 20))
                stab_topk = int(params.get("stability_topk", 20))
                stab_kind = str(params.get("stability_kind", "both"))
                stab_base_seed = int(params.get("stability_base_seed", 1234))
                stab_epochs = int(params.get("stability_epochs", 300))
                stab_lr = float(params.get("stability_lr", 0.05))

                print("\n🧪 [Stability] running reproducibility test...")
                print(f"    positions={stab_positions}, runs={stab_runs}, topk={stab_topk}, kind={stab_kind}, base_seed={stab_base_seed}")

                df_runs, df_pairs, df_sum = run_maskopt_stability_test(
                    model=model,
                    input_graphs=input_graphs,
                    target_node_idx=int(target_node_global_idx),
                    positions=stab_positions,
                    feature_names=feature_names,
                    node_to_idx=node_to_idx,
                    device=device,
                    n_runs=stab_runs,
...
def main():
    ...
    ap.add_argument("--xai_only", action="store_true",
        ...
        # ---- Stability test CLI ----
    ap.add_argument("--stability_test", action="store_true",
                    help="Run reproducibility (stability) test: repeat MaskOpt with different seeds and compare top-k overlap.")
    ap.add_argument("--stability_runs", type=int, default=20,
                    help="Number of repeated MaskOpt runs per (node, month).")
    ap.add_argument("--stability_positions", type=str, default="0,1,2",
                    help="Comma-separated explain_pos list to compare (e.g., '0,1,2' for Jan/Feb/Mar).")
    ap.add_argument("--stability_topk", type=int, default=20,
                    help="Top-k for overlap@k / Jaccard / Kendall.")
    ap.add_argument("--stability_kind", choices=["feat","edge","both"], default="both",
                    help="Compare stability for features, edges(neighbors), or both.")
    ap.add_argument("--stability_base_seed", type=int, default=1234,
                    help="Base seed; actual seeds are base_seed + run_index.")
    ap.add_argument("--stability_epochs", type=int, default=300,
                    help="MaskOpt epochs for stability runs.")
    ap.add_argument("--stability_lr", type=float, default=0.05,
                    help="MaskOpt learning rate for stability runs.")
    ...
    base_params = {
        ...
        "xai_attn_min_w": 0.0,
        # --- Stability test params (optional) ---
        "stability_test": bool(args.stability_test),
        "stability_runs": int(args.stability_runs),
        "stability_positions": str(args.stability_positions),
        "stability_topk": int(args.stability_topk),
        "stability_kind": str(args.stability_kind),
        "stability_base_seed": int(args.stability_base_seed),
        "stability_epochs": int(args.stability_epochs),
        "stability_lr": float(args.stability_lr),
