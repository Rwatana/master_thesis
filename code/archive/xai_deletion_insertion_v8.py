...
def _select_device(requested: str):
    """Choose a safe torch.device for this environment.
    ...
    accepted examples:
      ...
      - "mps"
      - "cuda", "cuda:0", "cuda:1", ...
      - "0" or "1" (treated as cuda:0 / cuda:1)

    If CUDA is requested but not available/compiled, we fall back to MPS (if available) then CPU.
    """
    import torch

    req = (requested or "auto").strip().lower()

    # convenience: "0" -> "cuda:0"
    if req.isdigit():
        req = f"cuda:{req}"

    def _cuda_ok() -> bool:
        # torch may be built without CUDA (common on macOS)
        try:
    ...
    if req.startswith("cuda"):
        ...
        # fallback
        if _mps_ok():
            print("[warn] CUDA requested but not available. Falling back to MPS.")
            return torch.device("mps")
...
def _filtered_kwargs(fn, kwargs: dict) -> dict:
    """Return kwargs that match the callable signature."""
    params = inspect.signature(fn).parameters
    return {k: v for k, v in kwargs.items() if k in params}


def _build_e2e_wrapper(
    *,
    model,
    input_graphs,
    target_node_idx: int,
    ...
    feat_mask_scope: str = "all",
    edge_mask_scope: str = "incident",
    edge_grouping: str = "neighbor",
    topk_edges: int = 512,
    topk_feats: int = 64,
):
    """Build E2EMaskOptWrapper across slightly different API versions."""
    sig = inspect.signature(E2EMaskOptWrapper.__init__)
    ...
    # handle naming differences: target_node_idx vs target_node
    if "target_node_idx" in params:
        kw["target_node_idx"] = target_node_idx
    elif "target_node" in params:
        kw["target_node"] = target_node_idx
    else:
        # last resort: positional after (model, input_graphs)
        try:
            return E2EMaskOptWrapper(model, input_graphs, target_node_idx, explain_pos, device)
        except TypeError as e:
            raise TypeError(f"E2EMaskOptWrapper signature not supported: {sig}") from e

    try:
        return E2EMaskOptWrapper(**_filtered_kwargs(E2EMaskOptWrapper.__init__, kw))
    except TypeError:
        # Some versions have __init__(self, model, input_graphs, target_node_idx, explain_pos, device, ...)
        # but may not accept extra kwargs; retry with just the essentials.
        essential = {"model": model, "input_graphs": input_graphs, "explain_pos": explain_pos, "device": device}
        if "target_node_idx" in params:
            essential["target_node_idx"] = target_node_idx
        elif "target_node" in params:
            essential["target_node"] = target_node_idx
        if "idx_to_node" in params:
            essential["idx_to_node"] = idx_to_node
        if "local2global" in params:
            essential["local2global"] = local2global
        return E2EMaskOptWrapper(**essential)


def _call_maskopt_e2e_explain(
    *,
    model,
    input_graphs,
    target_node_idx: int,
    ...
    """Call maskopt_e2e_explain across slightly different API versions."""
    sig = inspect.signature(maskopt_e2e_explain)
    params = sig.parameters
    ...
    if "target_node_idx" in params:
        ...
        raise TypeError(f"maskopt_e2e_explain signature not supported: {sig}")

    return maskopt_e2e_explain(**_filtered_kwargs(maskopt_e2e_explain, kw))
...
def _normalize_state_dict_keys(state: dict, model) -> dict:
    """Try to make checkpoint keys match model.state_dict() keys.

    Handles common wrappers:
      - DistributedDataParallel: 'module.' prefix
      - Training wrappers: 'model.' / 'net.' prefix

    Returns the candidate mapping with the highest key-overlap with model keys.
    """
    ...
    if set(state_keys) == set(model_keys):
        return state
    ...
    candidates = [state]
    for p in ("module.", "model.", "net.", "backbone.", "encoder."):
        candidates.append(_strip_prefix(state, p))

    # Some checkpoints are missing 'module.' while model has it (rare)
    if model_keys and all(k.startswith("module.") for k in model_keys):
        candidates.append({("module." + k if not k.startswith("module.") else k): v for k, v in state.items()})

    mk = set(model_keys)
    best = state
    best_score = -1
    for cand in candidates:
        score = len(set(cand.keys()) & mk)
        if score > best_score:
            best, best_score = cand, score

    if best_score <= 0:
        # last resort: try removing a single leading segment
        best2 = {}
        for k, v in state.items():
            k2 = k.split(".", 1)[-1] if "." in k else k
            best2[k2] = v
        score2 = len(set(best2.keys()) & mk)
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default="auto",
        help="torch device: auto / cpu / mps / cuda:0 (or just '0')",
    )
    ...
    for pos in positions:
        ...
        for node_id in target_nodes:
            ...
            if args.importance_source == "maskopt":
                df_feat, df_edge, meta = _call_maskopt_e2e_explain(
                    model=model,
                    input_graphs=monthly_graphs,
                    target_node_idx=node_id,
                    explain_pos=pos,
