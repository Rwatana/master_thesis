...
import csv       # ✅ CSVファイルへの書き込みのために追加
...
# MODEL_SAVE_PATH = f'influencer_rank_model_{time.strftime("%Y%m%d")}_rich_features_2017_3rd.pth' # ✅ 動的に設定するため削除
...
def train_and_save_model(model_save_path):
    ...
    LEARNING_RATE = 0.001
    DROPOUT_PROB = 0.5
    NUM_EPOCHS = 200 # NOTE: 100回実行するならエポック数は少なくても良いかもしれません
    ...
    if not END_TO_END_TRAINING:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for batch_indices, batch_true_scores in pbar:
                optimizer.zero_grad()
...
def run_inference(model_save_path):
    ...
    print(df_results.sort_values(by='Predicted_Score', ascending=False)[['Username', 'Predicted_Score', 'True_Score']].head(20).to_string(index=False))
    # ... (その他の表示) ...
    ...
    results_dict.update(ndcg_results) # NDCGの結果を辞書に追加
    return results_dict
...
# ✅ 変更点: mainブロックを全面的に書き換え
if __name__ == '__main__':
    # --- 1. 引数の設定 ---
    parser = argparse.ArgumentParser(description="Train and evaluate the InfluencerRank model.")
    parser.add_argument('--run_id', type=str, required=True, help='A unique identifier for this run (e.g., "run_1").')
    parser.add_argument('--results_file', type=str, default='experiment_results.csv', help='Path to the CSV file to save results.')
    ...
    # モデルを保存するディレクトリを作成 (なければ)
    models_dir = 'models'
    os.makedirs(models_dir, exist_ok=True)
    model_save_path = os.path.join(models_dir, f'model_{args.run_id}.pth')

    # --- 3. 学習と推論の実行 ---
    train_and_save_model(model_save_path=model_save_path)
    results = run_inference(model_save_path=model_save_path)
    ...
    if results:
        ...
        results_to_save = {
            ...
            **results # results辞書を展開して結合
        }
        
        # ファイルが存在しない場合はヘッダーを書き込む
        file_exists = os.path.isfile(args.results_file)
        with open(args.results_file, 'a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=results_to_save.keys())
            if not file_exists:
                writer.writeheader()
            writer.writerow(results_to_save)
        
        print(f"\n✅ Results for {args.run_id} saved to '{args.results_file}'")

    print(f"======= Finished Run ID: {args.run_id} =======")