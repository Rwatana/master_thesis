...
# Persist sweep results (params + metrics) to CSV after each run for easy offline review.
try:
    rows = []
    for r in all_results:
        ...
        for mk, mv in (r.metrics or {}).items():
            try:
                ...
                row[f"metric_{mk}"] = mv
        # params
        for pk, pv in (r.params or {}).items():
            if isinstance(pv, (dict, list, tuple)):
                row[f"param_{pk}"] = json.dumps(pv, ensure_ascii=False)
            else:
                row[f"param_{pk}"] = pv
        rows.append(row)
    pd.DataFrame(rows).to_csv(sweep_csv_path, index=False)
    print(f"[Sweep] wrote {sweep_csv_path.resolve()}")
except Exception as e:
