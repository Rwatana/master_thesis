...
def _parse_pre_torch_args():
    ...
    # XAI / MLflow / model loading options
    p.add_argument("--xai_only", action="store_true", help="Skip training; only run XAI (requires model).")
    p.add_argument("--mlflow_tracking_uri", type=str, default=None, help="MLflow tracking URI (e.g., sqlite:///mlflow.db).")
    p.add_argument("--mlflow_experiment_name", type=str, default=None, help="MLflow experiment name.")
    p.add_argument("--mlflow_model_run_id", type=str, default=None, help="MLflow run_id to load model artifact from.")
    p.add_argument("--model_pth_path", type=str, default=None, help="Local .pth path (state_dict) to load. Overrides --mlflow_model_run_id if set.")
    p.add_argument("--plot_scale_0_1", action="store_true", help="Scale importance/impact to [0,1] for plots.")
...
def maskopt_e2e_explain(
    ...
    try:
        cli = PRE_ARGS  # parsed before torch import
    except Exception:
        cli = None

    if cli is not None:
        if getattr(cli, "xai_only", False):
            params["XAI_ONLY"] = True
        if getattr(cli, "plot_scale_0_1", False):
            params["PLOT_SCALE_0_1"] = True

        if getattr(cli, "mlflow_tracking_uri", None):
            params["MLFLOW_TRACKING_URI"] = cli.mlflow_tracking_uri
        if getattr(cli, "mlflow_experiment_name", None):
            params["MLFLOW_EXPERIMENT_NAME"] = cli.mlflow_experiment_name
        if getattr(cli, "mlflow_model_run_id", None):
            params["MLFLOW_MODEL_RUN_ID"] = cli.mlflow_model_run_id
        if getattr(cli, "model_pth_path", None):
            params["MODEL_PTH_PATH"] = cli.model_pth_path
...
def run_experiment(params, graphs_data, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        if params.get("XAI_ONLY", False):
            ...
            local_pth = params.get("MODEL_PTH_PATH", None)
            if local_pth:
                if not os.path.isfile(local_pth):
                    raise FileNotFoundError(f"--model_pth_path not found: {local_pth}")
                loaded_model_run_id = f"local:{os.path.basename(local_pth)}"
                loaded_model_local_path = local_pth
                print(f"[XAI] Using local .pth: run_id={loaded_model_run_id} path={loaded_model_local_path}")
            else:
                loaded_model_run_id, loaded_model_local_path = mlflow_download_model_artifact(
                    tracking_uri=tracking_uri,
                    experiment_name=exp_name,
                    artifact_basename=artifact_basename,
                    run_id_hint=loaded_model_run_id,
                    dst_dir=os.path.join("mlflow_downloads", "models"),
                )
