...
def maskopt_e2e_explain(
    ...
    tag="pos_0",
    # extra knobs
    impact_reference="unmasked",  # "masked" | "unmasked" | "both"
    impact_fill="mean",  # "zero" | "mean" (feature ablation baseline for Score_Impact(unmasked))
    budget_feat=None,
    budget_edge=None,
    budget_weight=0.0,
    eps_abs_feat=1e-9,
    ...
    # Defaults to avoid UnboundLocalError in rare failure cases
    pred_unmasked = float(orig_pred)
    pred_masked = float(orig_pred)
    if feat_gate is not None and feat_gate.numel() > 0:
        feat_np = feat_gate.detach().cpu().numpy()
        # Evaluate impacts for ALL features (not just top-k), so we can rank/plot across the full set.
        feat_order = np.argsort(feat_np)[::-1]  # high-importance first (for readability)

        base_f_unmasked = ones_feat.clone()
        base_e_unmasked = ones_edge.clone() if ones_edge is not None else None
        base_f_masked = feat_gate.detach().clone()
        base_e_masked = edge_gate.detach().clone() if edge_gate is not None else None

        # Baseline predictions (computed once)
        with cudnn_ctx:
            if impact_reference in ('unmasked', 'both'):
                pred_unmasked = float(wrapper.predict_with_gates(base_f_unmasked, base_e_unmasked).item())
            if impact_reference in ('masked', 'both'):
                pred_masked   = float(wrapper.predict_with_gates(base_f_masked,   base_e_masked  ).item())

        # Mean-replacement baseline for feature ablation (unmasked reference)
        x_base_unmasked = None
        x_mean_unmasked = None
        if impact_reference in ('unmasked', 'both') and str(impact_fill).lower() == 'mean':
            x_base_unmasked = wrapper.x_exp.detach()
            x_mean_unmasked = x_base_unmasked.mean(dim=0)

        for j in feat_order:
            imp = float(feat_np[j])
            refs = []  # list of (ref_key, score_impact, direction_str, ablated_pred)

            # --- Unmasked reference impact ---
            if impact_reference in ('unmasked', 'both'):
                with cudnn_ctx:
                    if str(impact_fill).lower() == 'mean':
                        # Replace the entire feature-dimension with its mean (keeps scale, removes information).
                        x_override = x_base_unmasked.clone()
                        x_override[:, j] = x_mean_unmasked[j]
                        pred_abl_unmasked = float(wrapper.predict_with_gates(base_f_unmasked, base_e_unmasked, x_override=x_override).item())
                    else:
                        # Zero-out via gate (multiplicative ablation)
                        ab_f = base_f_unmasked.clone()
                        ab_f[j] = 0.0
                        pred_abl_unmasked = float(wrapper.predict_with_gates(ab_f, base_e_unmasked).item())
                diff_unmasked = float(pred_unmasked - pred_abl_unmasked)
                refs.append(('unmasked', diff_unmasked, _direction(diff_unmasked, pred_unmasked), pred_abl_unmasked))

            # --- Masked reference impact ---
            if impact_reference in ('masked', 'both'):
                with cudnn_ctx:
                    ab_fm = base_f_masked.clone()
                    ab_fm[j] = 0.0
                    pred_abl_masked = float(wrapper.predict_with_gates(ab_fm, base_e_masked).item())
                diff_masked = float(pred_masked - pred_abl_masked)
                refs.append(('masked', diff_masked, _direction(diff_masked, pred_masked), pred_abl_masked))
            ...
            # Keep which baseline we used for unmasked ablation (useful for analysis)
            if impact_reference in ('unmasked', 'both'):
                row['ImpactFill(unmasked)'] = str(impact_fill).lower()
            for key, diff, direction, pred_abl in refs:
                row[f'Score_Impact({key})'] = diff
                row[f'Direction({key})'] = direction
                row[f'AblatedScore({key})'] = pred_abl
    ...
    # Edge rows
    edge_rows = []
    # --- Edge group name restoration (edge_group_XXX -> actual edge endpoints) ---
    idx_to_node = {v: k for (k, v) in node_to_idx.items()} if isinstance(node_to_idx, dict) else {}

    def _edge_label_and_endpoints(gidx: int):
        """Return (label, eidx, src_name, dst_name, neighbor_name)."""
        if getattr(wrapper, "ei_exp", None) is None or wrapper.ei_exp.numel() == 0:
            return (f"edge_group_{gidx}", None, None, None, None)

        # Param index -> actual edge index in wrapper.ei_exp
        if getattr(wrapper, "edge_mask_scope", "incident") == "incident" and hasattr(wrapper, "incident_edge_idx"):
            if gidx >= int(wrapper.incident_edge_idx.numel()):
                return (f"edge_group_{gidx}", None, None, None, None)
            eidx = int(wrapper.incident_edge_idx[gidx].item())
        else:
            eidx = int(gidx)

        src_local = int(wrapper.ei_exp[0, eidx].item())
        dst_local = int(wrapper.ei_exp[1, eidx].item())

        if getattr(wrapper, "local2global", None) is not None:
            src_gid = int(wrapper.local2global[src_local].item())
            dst_gid = int(wrapper.local2global[dst_local].item())
        else:
            src_gid, dst_gid = src_local, dst_local

        src_name = idx_to_node.get(src_gid, f"node_{src_gid}")
        dst_name = idx_to_node.get(dst_gid, f"node_{dst_gid}")
        tgt_gid = int(getattr(wrapper, "target_global", -1))
        tgt_name = idx_to_node.get(tgt_gid, f"node_{tgt_gid}")

        # neighbor = other endpoint if this is incident to target
        neighbor_gid = None
        if src_gid == tgt_gid:
            neighbor_gid = dst_gid
        elif dst_gid == tgt_gid:
            neighbor_gid = src_gid

        if neighbor_gid is not None:
            neighbor_name = idx_to_node.get(neighbor_gid, f"node_{neighbor_gid}")
            label = f"{tgt_name}--{neighbor_name} (e{eidx})"
        else:
            neighbor_name = None
            label = f"{src_name}->{dst_name} (e{eidx})"
    ...
    if edge_gate is not None and edge_gate.numel() > 0:
        ...
        edge_order = np.argsort(edge_np)[::-1]  # ALL edges (not just top-k)
        for gidx in edge_order:
            ...
            edge_label, eidx, src_name, dst_name, neighbor_name = _edge_label_and_endpoints(gidx)
            row = {
                "Type": "Edge",
                "Name": edge_label,
                "EdgeIdx": eidx,
        ...
        # --- Add baseline score + impact percentage columns ---
        try:
            base_orig = float(orig)
            base_unmasked = float(pred_unmasked)
            base_masked = float(pred_masked)

            def _add_score_cols(df):
                if df is None or df.empty:
                    return df
                df = df.copy()
                df["OrigScore"] = base_orig
                df["RefScore(unmasked)"] = base_unmasked
                df["RefScore(masked)"] = base_masked
                for ref, denom in [("unmasked", base_unmasked), ("masked", base_masked)]:
                    col = f"Score_Impact({ref})"
                    if col in df.columns:
                        if abs(denom) < 1e-12:
                            df[f"ImpactPct({ref})"] = np.nan
                        else:
                            df[f"ImpactPct({ref})"] = 100.0 * df[col].astype(float) / denom
                # Ranks for scatter plots: Importance-rank vs Impact-rank
                df["ImportanceRank"] = df["Importance"].rank(ascending=False, method="min").astype(int)
                for ref in ("unmasked", "masked"):
                    col = f"Score_Impact({ref})"
                    if col in df.columns:
                        df[f"ImpactAbs({ref})"] = df[col].abs()
                        df[f"ImpactRank({ref})"] = df[f"ImpactAbs({ref})"].rank(ascending=False, method="min").astype(int)
                return df

            df_feat = _add_score_cols(df_feat)
            df_edge = _add_score_cols(df_edge)
        except Exception as _e:
            print(f"⚠️ Failed to add impact percent columns: {_e}")
...
def mlflow_log_maskopt_plots(df_feat, df_edge, meta=None, tag="pos_0", topk_feat=15, topk_edge=15, artifact_path="xai", fname_prefix="maskopt"):
    ...
    # Rank scatter plots: ImportanceRank vs ImpactRank (for analysis of ranking agreement)
    try:
        for _df, _kind in ((df_feat, 'feat'), (df_edge, 'edge')):
            if _df is None or _df.empty:
                continue
            # Prefer the requested reference, but fall back gracefully if columns are missing
            _key = 'unmasked' if str(impact_reference).lower() in ('unmasked', 'unmask', 'both') else 'masked'
            _rank_col = 'ImpactRank(' + _key + ')'
            _impact_col = 'Score_Impact(' + _key + ')'
            if _rank_col not in _df.columns:
                if 'ImpactRank(unmasked)' in _df.columns:
                    _key = 'unmasked'
                elif 'ImpactRank(masked)' in _df.columns:
                    _key = 'masked'
                _rank_col = 'ImpactRank(' + _key + ')'
                _impact_col = 'Score_Impact(' + _key + ')'
            _xcol = 'ImportanceRank'
            _ycol = _rank_col
            if _xcol not in _df.columns or _ycol not in _df.columns:
                continue
            plt.figure(figsize=(5, 4))
            plt.scatter(_df[_xcol].values, _df[_ycol].values, s=10, alpha=0.7)
            plt.xlabel('Importance rank (1=highest)')
            plt.ylabel('Impact rank (1=largest |Δscore|) [' + _key + ']')
            plt.title(f"{_kind}: rank scatter (node={node_id}, pos={explain_pos})")
            plt.tight_layout()
            _out_path = os.path.join(out_dir, f"{tag}_{run_tag}_maskopt_rank_scatter_{_kind}_{_key}.png")
            plt.savefig(_out_path, dpi=200)
            plt.close()
            mlflow.log_artifact(_out_path, artifact_path=artifact_subdir)
            # Metric: Spearman between Importance and |Score_Impact| (same reference key)
            if _impact_col in _df.columns:
                _rho, _p = spearmanr(_df['Importance'].values, _df[_impact_col].abs().values)
                if np.isfinite(_rho):
                    mlflow.log_metric(f"{tag}_{run_tag}_maskopt_spearman_importance_vs_absimpact_{_kind}_{_key}", float(_rho))
    except Exception as e:
        print(f"[WARN] rank scatter plot failed: {e}")
