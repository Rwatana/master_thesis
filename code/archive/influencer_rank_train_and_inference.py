...
#         engagement_rates = monthly_posts_period.groupby('username').agg(avg_likes=('likes', 'mean')).reset_index()
...
# ✅✅✅ --- メトリック計算を柔軟にした `prepare_graph_data` --- ✅✅✅
...
#     # --- データ読み込み (変更なし) ---
#     df_posts = pd.read_csv(PREPROCESSED_FILE, parse_dates=['datetime'], low_memory=False)
#     # 🚨 注意: 'comments' 列が preprocessed_posts_with_metadata.csv に必要です
...
#             mask = merged_data['followers'] > 0
#             merged_data.loc[mask, 'engagement'] = merged_data.loc[mask, 'numerator'] / merged_data.loc[mask, 'followers']
#         else: # デフォルトは 'posts'
...
# ✅✅✅ --- 新しく追加した推論専用の関数 --- ✅✅✅
# def run_inference():
#     """学習済みモデルをロードし、最新データで推論（予測）だけを行う関数"""
...
#     df_posts = pd.read_csv(PREPROCESSED_FILE, parse_dates=['datetime'], low_memory=False)
#     latest_date = sorted(df_posts['datetime'].dt.to_period('M').dt.start_time.unique())[-1]
#     # 最新の11ヶ月分のデータを入力として使用
#     predict_input_graphs, predict_indices, node_to_idx = prepare_graph_data(end_date=latest_date, num_months=11)

#     # 3. 予測の実行
#     model.eval()
#     with torch.no_grad():
#         sequence_embeddings = torch.stack([model.gcn_encoder(g.x, g.edge_index) for g in tqdm(predict_input_graphs, desc="GCN Encoding for Inference")])
...
#     df_results = df_results.sort_values(by='Predicted_Rank').set_index('Predicted_Rank')
    
#     print("\n🏆 --- Top 20 Predicted Influencers --- 🏆")
#     print(df_results[['Username', 'Predicted_Score']].head(20))
#     end_time = time.time()
