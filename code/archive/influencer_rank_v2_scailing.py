...
from torch.nn import GRU, Linear, ReLU, Tanh, Dropout, LeakyReLU
...
MODEL_SAVE_PATH = f'influencer_rank_model_{time.strftime("%Y%m%d")}_rich_features_scailing.pth'
...
def prepare_graph_data(end_date, num_months=12, metric_numerator='likes', metric_denominator='posts'):
    ...
    # ✅✅✅ 変更点: 'Category' カラムを読み込むようにリネームを追加 ✅✅✅
    df_influencers.rename(columns={
        '#Followers': 'followers', 
        '#Followees': 'followees', 
        '#Posts': 'posts', 
        'Username': 'username',
        'Category': 'category' # カテゴリカラムを追加
    }, inplace=True)
    ...
    # カテゴリ情報をOne-Hotエンコーディング
    category_dummies = pd.get_dummies(profile_features['category'], prefix='cat', dummy_na=True) # NaNもカテゴリとして扱う
    ...
    # (以降の動的特徴量の処理は変更ありません)
    # --- 2. 動的特徴量 (Dynamic Features) の元データ準備 ---
    ...
    for snapshot_date in tqdm(pd.date_range(start_date, end_date, freq='ME'), desc="Building monthly graphs"):
        ...
        # ✅ 標準化のステップを追加
        feature_values = snapshot_features.values
        # スケーラーをインスタンス化
...
class InfluencerRankModel(nn.Module):
    def __init__(self, feature_dim, gcn_dim, rnn_dim, num_gcn_layers=2, dropout_prob=0.5):
        ...
        self.predictor = nn.Sequential(Linear(rnn_dim, 16), LeakyReLU(), Dropout(dropout_prob), Linear(16, 1))
