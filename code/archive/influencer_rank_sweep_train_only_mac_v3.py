...
def _parse_pre_torch_args():
    ...
    p.add_argument("--mlflow_tracking_uri", type=str, default=None,
                   help="MLflow tracking URI (e.g., sqlite:///mlflow.db or file:./mlruns). If omitted, uses env MLFLOW_TRACKING_URI or defaults.")
    p.add_argument("--mlflow_experiment_name", type=str, default="InfluencerRankSweep",
                   help="MLflow experiment name.")
    p.add_argument("--outdir", type=str, default="out_sweep",
                   help="Output directory for local copies (models, csv, plots).")

    # --- Train / sweep ---
    p.add_argument("--epochs", type=int, default=50, help="Training epochs for each run.")
    p.add_argument("--train_only", action="store_true",
                   help="Skip XAI; keep only training + evaluation (true vs pred scatter + CSV).")

    p.add_argument("--sweep", action="store_true", help="Enable hyperparameter sweep (cartesian product).")
    p.add_argument("--sweep_json", type=str, default=None,
                   help="Path to JSON file that defines a sweep grid, e.g. {'LR':[...],'GCN_DIM':[...]}")

    p.add_argument("--sweep_lr", nargs="*", type=float, default=None, help="List of learning rates to sweep.")
    p.add_argument("--sweep_weight_decay", nargs="*", type=float, default=None, help="List of weight_decay to sweep.")
    p.add_argument("--sweep_gcn_dim", nargs="*", type=int, default=None, help="List of GCN hidden dims to sweep.")
    p.add_argument("--sweep_rnn_dim", nargs="*", type=int, default=None, help="List of RNN hidden dims to sweep.")
    p.add_argument("--sweep_num_gcn_layers", nargs="*", type=int, default=None, help="List of #GCN layers to sweep.")
    p.add_argument("--sweep_dropout", nargs="*", type=float, default=None, help="List of dropout probs to sweep.")
    p.add_argument("--sweep_batch_size", nargs="*", type=int, default=None, help="List of batch sizes to sweep.")
    p.add_argument("--sweep_list_size", nargs="*", type=int, default=None, help="List of list sizes to sweep (ranking loss).")
    p.add_argument("--max_runs", type=int, default=0, help="If >0, cap the number of sweep runs executed.")
...
class E2EMaskOptWrapper(nn.Module):
    ...
    def _prepare_explain_graph(self):
        ...
        data_full = self.graphs[self.explain_pos]
        x_full = data_full.x
...
def maskopt_e2e_explain(
    ...
    def _edge_label_and_endpoints(edge_k, ei, target_node_idx, node_to_idx, local2global=None):
        ...
        if local2global is not None:
            ...
            t = int(local2global[int(target_node_idx)].item())
        else:
            s, d, t = s_local, d_local, int(target_node_idx)

        sname = _node_name(node_to_idx, s)
        dname = _node_name(node_to_idx, d)
...
def run_experiment(params, graphs_data, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=params["LR"], weight_decay=float(params.get("WEIGHT_DECAY", 0.0)))
        ...
        # --- Model load / training control (XAI-only mode) ---
        loaded_model_run_id = None
        loaded_model_local_path = None
        if params.get("XAI_ONLY", False):
            tracking_uri = params.get("MLFLOW_TRACKING_URI", "sqlite:///mlflow.db")
            ...
            print(f"[MLflow] Using model run_id: {loaded_model_run_id}")
            print(f"[MLflow] Model artifact saved at: {loaded_model_local_path}")
            if loaded_model_run_id is None or loaded_model_local_path is None:
                print("[MLflow] WARNING: --xai_only requested, but no MLflow model artifact was found.")
                print(f"[MLflow]          experiment={exp_name} tried_artifacts=['model/model_state_dict.pth', 'model_state_dict.pth', 'model.pth']")
                print("[MLflow]          Falling back to TRAINING in this run and logging the trained model artifact.")
                params["XAI_ONLY"] = False
            else:
                state = torch.load(loaded_model_local_path, map_location="cpu")
                ...
                mlflow.log_param("loaded_model_artifact", "model/model_state_dict.pth")
        xai_only = params.get("XAI_ONLY", False)
        epochs_to_run = 0 if xai_only else int(params["EPOCHS"])
        if xai_only:
            print("[XAI_ONLY] Skipping training loop; using loaded model for inference/XAI.")
            model.eval()
        else:
        ...
        # --- Persist trained model (so future runs can reuse without retraining) ---
        # NOTE: In some variants this logging was accidentally invoked multiple times.
        # Guard with an in-process flag to avoid duplicate artifacts/prints.
        if epochs_to_run > 0 and not params.get('_MODEL_LOGGED_THIS_RUN', False):
            try:
                model_artifact_local = os.path.join(os.getcwd(), 'model_state_dict.pth')
                torch.save(model.state_dict(), model_artifact_local)
                mlflow.log_artifact(model_artifact_local, artifact_path='model')
                print(f"[MLflow] ✅ Saved trained model artifact: run_id={run_id} -> model/model_state_dict.pth")
                print(f"[MLflow]    local_path={model_artifact_local}")
                mlflow.log_param('trained_model_run_id', run_id)
                mlflow.log_param('trained_model_artifact', 'model/model_state_dict.pth')
                params['_MODEL_LOGGED_THIS_RUN'] = True
            except Exception as e:
                print(f"[MLflow] WARNING: Failed to log trained model artifact: {e}")
        print("\nStarting Inference...")
        model.eval()
        ...
        # --- Save per-run (true, pred) table + basic range stats ---
        try:
            outdir = params.get("OUTDIR", "out_sweep")
            os.makedirs(outdir, exist_ok=True)
            idx_to_node = {int(v): k for k, v in node_to_idx.items()}
            df_pred = pd.DataFrame({
                "global_idx": influencer_indices,
                "node_id": [idx_to_node.get(int(i), str(i)) for i in influencer_indices],
                "true_score": true_scores,
                "pred_score": predicted_scores,
            })
            pred_csv = os.path.join(outdir, f"pred_true_{run_id}.csv")
            df_pred.to_csv(pred_csv, index=False)
            mlflow.log_artifact(pred_csv, artifact_path="eval")
            print(f"[Eval] Saved true/pred table: {pred_csv}")

            import numpy as _np
            def _stats(arr, name):
                arr=_np.asarray(arr, dtype=float)
                qs=_np.percentile(arr, [0,1,5,25,50,75,95,99,100])
                print(f"[Eval] {name} min={qs[0]:.6f} p1={qs[1]:.6f} p5={qs[2]:.6f} p25={qs[3]:.6f} "
                      f"p50={qs[4]:.6f} p75={qs[5]:.6f} p95={qs[6]:.6f} p99={qs[7]:.6f} max={qs[8]:.6f}")
            _stats(true_scores, "true_score")
            _stats(predicted_scores, "pred_score")
        except Exception as e:
            print(f"[Eval] ⚠️ Failed to save pred table/stats: {e}")

        # Skip XAI completely (train + eval only)
        if params.get("SKIP_XAI", False):
            final_test_metrics = {
                "MAE": float(mae),
                "RMSE": float(rmse),
...
def main():
    ...
    experiment_name, experiment_id = setup_mlflow_experiment(
        experiment_base_name=(PRE_ARGS.mlflow_experiment_name or os.environ.get("MLFLOW_EXPERIMENT_NAME", "InfluencerRankSweep")),
        tracking_uri=(PRE_ARGS.mlflow_tracking_uri or os.environ.get("MLFLOW_TRACKING_URI")),
        local_artifact_dir=os.environ.get("MLFLOW_ARTIFACT_DIR", "mlruns_artifacts"),
    )

    os.makedirs(PRE_ARGS.outdir, exist_ok=True)
    ...
    base_params["OUTDIR"] = PRE_ARGS.outdir
    # This script is intended for training-sweep + eval only.
    base_params["SKIP_XAI"] = True
    ...
    # --- Build sweep grid from CLI ---
    if PRE_ARGS.sweep_json:
        try:
            with open(PRE_ARGS.sweep_json, 'r', encoding='utf-8') as f:
                sweep_grid = json.load(f)
            if not isinstance(sweep_grid, dict):
                raise ValueError('sweep_json must contain a dict mapping param -> list')
        except Exception as e:
            raise SystemExit(f"[Sweep] Failed to load sweep_json '{PRE_ARGS.sweep_json}': {e}")

    if PRE_ARGS.sweep:
        if PRE_ARGS.sweep_lr: sweep_grid['LR'] = PRE_ARGS.sweep_lr
        if PRE_ARGS.sweep_weight_decay: sweep_grid['WEIGHT_DECAY'] = PRE_ARGS.sweep_weight_decay
        if PRE_ARGS.sweep_gcn_dim: sweep_grid['GCN_DIM'] = PRE_ARGS.sweep_gcn_dim
        if PRE_ARGS.sweep_rnn_dim: sweep_grid['RNN_DIM'] = PRE_ARGS.sweep_rnn_dim
        if PRE_ARGS.sweep_num_gcn_layers: sweep_grid['NUM_GCN_LAYERS'] = PRE_ARGS.sweep_num_gcn_layers
        if PRE_ARGS.sweep_dropout: sweep_grid['DROPOUT_PROB'] = PRE_ARGS.sweep_dropout
        if PRE_ARGS.sweep_batch_size: sweep_grid['BATCH_SIZE'] = PRE_ARGS.sweep_batch_size
        if PRE_ARGS.sweep_list_size: sweep_grid['LIST_SIZE'] = PRE_ARGS.sweep_list_size
  # or define grid
