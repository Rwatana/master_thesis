...
# Add the date to the model save path to differentiate versions
...
def main():
    ...
    # --- バッチ処理のための事前計算 (CPU上で実行) ---
    print("Pre-computing node embeddings on CPU...")
    model.eval() # 勾配計算をオフ
    with torch.no_grad():
    ...
    for epoch in range(num_epochs):
        ...
        for batch_indices, batch_true_scores in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            final_user_representation = model.attentive_rnn(dummy_sequence)
