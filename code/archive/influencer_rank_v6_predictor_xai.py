...
from sklearn.preprocessing import StandardScaler # 貢献度分析のために追加
...
def analyze_and_visualize_attention(top_n=20):
    ...
    # 貢献度（final_representation）をフックで取得するための準備
    contributions_data = {}
    def get_final_rep_hook(module, input, output):
        # AttentiveRNN の forward の戻り値は (context_vector, attention_weights)
        # 欲しいのは context_vector (output[0])
        output[0].retain_grad() # 勾配を保持するよう設定
        contributions_data['final_rep'] = output[0]

    # モデルの attutive_rnn モジュールにフックを登録
    hook = model.attentive_rnn.register_forward_hook(get_final_rep_hook)
    
    # -----------------------------------------------------------------
    # 勾配計算を有効にしてフォワードパスを「再実行」
    # -----------------------------------------------------------------
    predicted_scores_for_grad, _ = model(inference_input_graphs, predict_indices)

    # df_sorted はアテンション分析で既に作成済み (Predicted_Score でソート済み)
    # df_sorted には元のインデックス 'index' が保持されている
    
    # TopNインフルエンサーのバッチ内インデックスを取得
    ...
    top_n_scores_sum.backward() # 勾配を計算
    
    # フックで保持した final_representation とその勾配を取得
    final_rep_all = contributions_data['final_rep']      # (全インフルエンサー数, 64)
    final_rep_grad_all = final_rep_all.grad  # (全インフルエンサー数, 64)
    
    # 使い終わったフックを削除
    hook.remove()

    # 貢献度 (Gradient * Input) を計算
    # TopNインフルエンサーのデータのみ抽出
    ...
    # 貢献度 = 入力 * 勾配
    contribution_scores = top_n_final_rep * top_n_final_rep_grad # Shape: (top_n, 64)
    ...
    # ヒートマップ用にスケーリング (StandardScaler を使用)
    ...
    plt.figure(figsize=(20, 10)) # 64次元を表示するため横長に
    sns.heatmap(
        df_contribution_scaled, 
        yticklabels=df_contribution_scaled.index, 
        cmap="vlag", # 0を中央とする発散色マップ (vlag or RdBu_r)
        center=0,     # 0を中心に色付け
