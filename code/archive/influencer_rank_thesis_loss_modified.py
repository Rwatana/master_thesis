...
def prepare_graph_data():
    ...
    try:
        ...
        # メモリ上でクリーンなファイルを作成してpandasで読み込む
    ...
    for snapshot_date in tqdm(pd.date_range(start_date, end_date, freq='ME'), desc="Building monthly graphs"): # freq='M' is deprecated
...
class GCNEncoder(nn.Module):
    ...
    def forward(self, x, edge_index): # 修正: シグネチャを呼び出しに合わせる
        layer_outputs = []
...
class ListwiseRankingLoss(nn.Module):
    ...
    def forward(self, pred_scores, true_scores):
        ...
        true_probs = F.softmax(true_scores, dim=0)
        loss = -torch.sum(true_probs * torch.log(pred_probs + 1e-9))
        return loss

# --- 4. Main Execution Block ---
def main():
    print("--- Training Paper-Aligned Model (CPU Full-Batch) ---")
