...
Example:
  python export_maskopt_all_importance.py --model-path influencer_rank_model_xxx.pth \
    --target-months pos_0,pos_1,pos_2 --targets all --devices 0
"""
...
# ---- pre-torch args: set CUDA_VISIBLE_DEVICES before importing torch ----
def _parse_pre_torch_args():
    p = argparse.ArgumentParser(add_help=False)
    p.add_argument("--visible", type=str, default=None,
                   help="Set CUDA_VISIBLE_DEVICES before torch import, e.g. '0' or '0,1'.")
    p.add_argument("--mps-fallback", action="store_true",
                   help="Set PYTORCH_ENABLE_MPS_FALLBACK=1 before torch import.")
...
def setup_device(args):
    """
    Device selection:
      - args.device == "auto": prefer CUDA -> MPS -> CPU
      - args.device == "cuda"/"mps"/"cpu": force it (error if unavailable)
    NVIDIA:
      - --visible sets CUDA_VISIBLE_DEVICES (useful on multi-GPU servers)
      - --cuda selects index within the visible set
    Mac (MPS):
      - --mps_fallback sets PYTORCH_ENABLE_MPS_FALLBACK=1 (optional)
    """
    # --- NVIDIA visible devices (only meaningful for CUDA) ---
    ...
    if want == "mps":
        ...
        if getattr(args, "mps_fallback", False):
            os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"
...
class E2EMaskOptWrapper(nn.Module):
    ...
    def _prepare_explain_graph(self):
        ...
        if self.edge_mask_scope != "incident":
            # not supported in this minimal exporter
    ...
    def _make_edge_weight(self, edge_gate):
        ...
        if self.edge_mask_scope == "incident":
            ...
            if self.edge_grouping == "neighbor":
                # edge_gate: [G], expand to incident edges by group id
...
def _device_from_str(s: str):
    s = s.strip().lower()
    if s in ("cpu", "mps"):
        return torch.device(s)
    if s.startswith("cuda:"):
        return torch.device(s)
    # "0" とか "1" とかは CUDA index とみなす
...
def run_export_for_targets(
    ...
    for tgt in tqdm(targets, desc=f"[{device_str}] targets", leave=False):
        for pos in explain_positions:
            ...
            best, pred_unmasked = optimize_gates(wrapper, cfg)
            feat_gate = best["feat"]
            edge_gate = best["edge"]
            ...
            tag = f"tgt_{tgt}_pos_{pos}_dev_{str(device_str).replace(':','_')}"
    ...
    sum_path = os.path.join(out_dir, f"summary_dev_{str(device_str).replace(':','_')}.csv")
...
def parse_args():
    ...
    p.add_argument("--target-months", type=str, default="pos_0,pos_1,pos_2",
                   help="Explain positions, e.g. 'pos_0,pos_1,pos_2' (these are indices in input_graphs).")
    ...
    p.add_argument("--visible", default=None,
                   help="Set CUDA_VISIBLE_DEVICES (e.g., 0 or 0,1). NVIDIA only.")
    p.add_argument("--mps-fallback", action="store_true",
                   help="Enable PYTORCH_ENABLE_MPS_FALLBACK=1")
    ...
    p.add_argument("--devices", type=str, default="auto",
                   help="Comma-separated cuda indices or 'cpu'. For multi-GPU parallel: '0,1,2'.")
...
def main():
    ...
    # ★devices を最終決定（auto を解決）
    if args.devices.strip().lower() == "auto":
        if dev.type == "cuda":
            devices = [str(dev.index if dev.index is not None else 0)]
        elif dev.type == "mps":
            devices = ["mps"]
        else:
            devices = ["cpu"]
    ...
    # input_graphs = monthly_graphs[:-1] has length T (e.g., 11)
    ...
    if args.targets == "all":
        ...
        # pick one hub in last input graph (Nov if Dec is target)
        g = monthly_graphs[-2]
        ei = g.edge_index
        # degree on CPU
        ...
        # topk: quick heuristic = first K influencers (you can replace with predicted topK)
        targets = list(map(int, influencer_indices))[: max(1, int(args.topk_targets))]

    devices = [d.strip() for d in args.devices.split(",") if d.strip()]
    # ★MPS/CPU は単体実行に寄せる
    if devices[0] in ("mps", "cpu"):
        sum_path = run_export_for_targets(devices[0], args, payload, targets, pos_list)
        print(f"[DONE] summary: {sum_path}")
        return 0
    # ---- multi-process only when multiple devices specified ----
    if len(devices) == 1:
        sum_path = run_export_for_targets(devices[0], args, payload, targets, pos_list)
        print(f"[DONE] summary: {sum_path}")
        return 0
    ...
    def _worker(dev, sub_targets, ret_list):
        sp = run_export_for_targets(dev, args, payload, sub_targets, pos_list)
        ret_list.append(sp)

    for dev, sub in zip(devices, chunks):
        if len(sub) == 0:
            continue
        p = ctx.Process(target=_worker, args=(dev, sub, ret))
        p.start()
        procs.append(p)
