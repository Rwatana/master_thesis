...
- 文脈条件付き生成分布 \(p(x*S\mid x*{\bar{S}})\) を仮定する（近傍からの統計推定など）。

#### 2.1 予測差分（Prediction Difference）

領域 \(S\) を文脈に従ってサンプル置換した入力 \(x^{(S)}\) に対し,
\[
\Delta*S^{(c)}(x) \,=\, f_c(x) \; - \; \mathbb{E}*{\tilde{x}_S \sim p(\cdot\mid x_{\bar{S}})} \big[ f_c(\,\tilde{x}_S,\; x_{\bar{S}}\,) \big].
\]
...
\[
\mathbb{E}[\cdot] \;\approx\; \frac{1}{M}\sum*{m=1}^{M} f_c\big(\tilde{x}^{(m)}\_S, x*{\bar{S}}\big).
\]
...
- **独立周辺化**: \(p(x*S\mid x*{\bar{S}}) \approx p(x_S)\) とする簡易近似。
- **文脈条件付き**: 近傍パッチからガウス/ガウシアン混合などで \(p(x*S\mid x*{\bar{S}})\) を推定してサンプル（自然画像の統計をより保つ）。
...
- **重要性**: クラス賛成/反対の根拠を区別でき,医用画像などで誤診リスクの可視化に有用。勾配ベース手法や Grad-CAM と補完的。

---

### 5. まとめ（要点）

- 予測差分: \(\Delta*S^{(c)} = f_c(x) - \mathbb{E}*{p(x*S\mid x*{\bar{S}})}[f_c(\tilde{x}_S, x_{\bar{S}})]\)。
