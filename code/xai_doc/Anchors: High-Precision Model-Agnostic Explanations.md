...
- Anchor の精度（条件が成り立つとき同じ予測になる確率）
  \[
  \text{Prec}(A) \,=\, \Pr\_{z\sim \mathcal{D}(\cdot\mid A)}\big[ f(z)=y \big].
  \]
- Anchor の被覆（条件がどの程度現れやすいか）
  \[
  \text{Cov}(A) \,=\, \Pr\_{z\sim \mathcal{D}}\big[ z \models A \big].
  \]
- 目的: 最小限の述語で \(\text{Prec}(A)\ge 1-\epsilon\) を満たしつつ,\(\text{Cov}(A)\) を最大化。

#### 推定と保証（サンプリング）

- サンプル \(\{z*i\}*{i=1}^n \sim \mathcal{D}(\cdot\mid A)\) に対し,経験精度 \(\hat{p} = \tfrac{1}{n}\sum \mathbf{1}[f(z_i)=y]\)。
