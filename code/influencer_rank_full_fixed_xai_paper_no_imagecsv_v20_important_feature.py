...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    def _select_nodes_for_heatmap(all_indices_nz, predicted_scores_nz, true_scores_nz,
                                strategy="pred_top", n=64, seed=0):
        rng = np.random.RandomState(int(seed))
        N = len(all_indices_nz)
        n = int(min(n, N))
        if n <= 0:
            return all_indices_nz[:0].astype(np.int64)

        if strategy == "pred_top":
            idx = np.argsort(-predicted_scores_nz)[:n]
        elif strategy == "true_top":
            idx = np.argsort(-true_scores_nz)[:n]
        elif strategy == "random":
            idx = rng.choice(np.arange(N), size=n, replace=False)
        else:
            # fallback
            idx = np.argsort(-predicted_scores_nz)[:n]
        return all_indices_nz[idx].astype(np.int64)

    def _select_positions_topk(attn_row, topk=3, min_w=0.0):
        a = np.asarray(attn_row).reshape(-1)
        order = np.argsort(-a)
        pos = []
        for i in order:
            if len(pos) >= int(topk):
                break
            if float(a[i]) >= float(min_w):
                pos.append(int(i))
        # 何も取れない場合の保険
        if len(pos) == 0:
            pos = [int(order[0])] if len(order) > 0 else [0]
        return pos
    ...
    N0 = int(monthly_graphs[0].num_nodes)

    # numpy化
    influencer_indices = np.asarray(influencer_indices)

    # もし influencer_indices が N0 を超えてたら global id っぽいので node_to_idx で引き直す
    if influencer_indices.size > 0 and int(np.max(influencer_indices.astype(np.int64))) >= N0:
        remapped = []
        missing = []
        for gid in influencer_indices.tolist():
            key = str(int(gid))  # ★ "1294420" みたいな文字列キーで入ってる想定
            if key in node_to_idx:
                remapped.append(int(node_to_idx[key]))
            else:
                missing.append(gid)

        if len(remapped) == 0:
            raise ValueError(
                f"influencer_indices look like global ids but couldn't remap via node_to_idx. "
                f"example_gid={influencer_indices[:5].tolist()}, N={N0}"
            )

        influencer_indices = np.array(remapped, dtype=np.int64)

    # 最終チェック
    if int(np.max(influencer_indices)) >= N0:
        raise ValueError(f"influencer_indices still out of range: max={int(np.max(influencer_indices))}, N={N0}")
    idx_to_node = {int(v): str(k) for k, v in node_to_idx.items()}
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        with torch.no_grad():
            ...
            # map dataset indices -> influencer-row
            all_idx = all_indices.to(device)                 # these are node indices in the graph
            all_local = global2local[all_idx]                # [Ntest] in 0..Ninf-1
            if int((all_local < 0).sum().item()) != 0:
                raise RuntimeError("test_dataset contains non-influencer indices (global2local == -1).")

            # gather in dataset order (CPU is OK; later you slice and .to(device))
            all_local_cpu = all_local.detach().cpu()
            f_seq = f_seq_inf.index_select(0, all_local_cpu)  # [Ntest,T,D]
            f_raw = f_raw_inf.index_select(0, all_local_cpu)  # [Ntest,T,P]
        ...
        def _resolve_usernames_to_ids(usernames, node_to_idx):
            """
            usernames: list[str]
            node_to_idx: dict[name -> global_id]
            return: (resolved_ids:list[int], resolved_names:list[str], missing_names:list[str])
            """
            resolved_ids = []
            resolved_names = []
            missing = []
            ...
            for u in usernames:
                ...
                # try variants (instagram: sometimes @prefix, case)
                cand = []
                cand.append(raw)
                if raw.startswith("@"):
                    cand.append(raw[1:])
                cand.append(raw.lower())
                if raw.startswith("@"):
                    cand.append(raw[1:].lower())

                gid = None
                hit_name = None
                for c in cand:
                    if c in node_to_idx:
                        gid = int(node_to_idx[c])
                        hit_name = c
                        break
                ...
                if gid is None:
                    ...
                    resolved_names.append(hit_name)

            # de-dup keep order
            seen = set()
            ids_uniq, names_uniq = [], []
            for gid, nm in zip(resolved_ids, resolved_names):
                if gid in seen:
                    continue
                seen.add(gid)
                ids_uniq.append(int(gid))
                names_uniq.append(str(nm))

            return ids_uniq, names_uniq, missing


        # ---- choose nodes for heatmap (HEATMAP_USERNAMES > HEATMAP_NODE_IDS > sampling)
        forced_usernames = _parse_str_list(params.get("HEATMAP_USERNAMES", None))

        if forced_usernames is not None:
            ids_req, names_hit, missing = _resolve_usernames_to_ids(forced_usernames, node_to_idx)

            # nonzero-filtered set only (because heatmap uses f_seq_nz / f_raw_nz)
            nz_set = set(int(x) for x in all_indices_nz.tolist())
            ids_final = [int(gid) for gid in ids_req if int(gid) in nz_set]

            # if all got filtered out, fallback
            if len(ids_final) == 0:
                sampled_nodes = _select_nodes_for_heatmap(
                    all_indices_nz, predicted_scores_nz, true_scores_nz,
                    strategy=HM_STRATEGY, n=HM_N, seed=seed0
                )
            else:
                sampled_nodes = np.array(ids_final, dtype=np.int64)

            # ---- log reproducibility
            # requested usernames (head + sha1)
            req_str = ",".join(forced_usernames)
            mlflow.log_param("heatmap_usernames_req_count", int(len(forced_usernames)))
            mlflow.log_param("heatmap_usernames_req_head", ",".join(forced_usernames[:50]))
            mlflow.log_param("heatmap_usernames_req_sha1", hashlib.sha1(req_str.encode("utf-8")).hexdigest())

            # missing usernames (head)
            if len(missing) > 0:
                mlflow.log_param("heatmap_usernames_missing_count", int(len(missing)))
                mlflow.log_param("heatmap_usernames_missing_head", ",".join(missing[:50]))

            # resolved ids
            ids_list = [int(x) for x in sampled_nodes.tolist()]
            ids_str = ",".join(map(str, ids_list))
            mlflow.log_param("heatmap_nodes_count", int(len(ids_list)))
            mlflow.log_param("heatmap_nodes_head", ",".join(map(str, ids_list[:50])))
            mlflow.log_param("heatmap_nodes_sha1", hashlib.sha1(ids_str.encode("utf-8")).hexdigest())

            # (任意) usernames全部をparamに分割して残す
            if bool(params.get("HEATMAP_LOG_USERNAMES_CHUNKED", False)):
                chunk = 180
                for i in range(0, len(req_str), chunk):
                    mlflow.log_param(f"heatmap_usernames_part_{i//chunk:03d}", req_str[i:i+chunk])

        else:
            # existing behavior (sampling)
            sampled_nodes = _select_nodes_for_heatmap(
                all_indices_nz, predicted_scores_nz, true_scores_nz,
                strategy=HM_STRATEGY, n=HM_N, seed=seed0
            )
        ...
        # -----------------------------
        # 6) Aggregate heatmap: save ONLY final heatmaps
        # -----------------------------
        do_heatmap = bool(params.get("DO_AGG_HEATMAP", True))
        if do_heatmap:
            try:
                HM_N = int(params.get("HEATMAP_SAMPLE_N", 64))
                HM_POS_TOPK = int(params.get("HEATMAP_POS_TOPK", 3))
                HM_POS_MINW = float(params.get("HEATMAP_POS_MIN_W", 0.02))
                HM_FEAT_TOPK = int(params.get("HEATMAP_TOPK_FEAT_PER_POS", 20))
                HM_SHOW_TOPF = int(params.get("HEATMAP_SHOW_TOP_FEATURES", 40))
                HM_STRATEGY = str(params.get("HEATMAP_SAMPLE_STRATEGY", "pred_top"))
                seed0 = int(params.get("SEED", 0))

                # ★保存するのはヒートマップPNGだけ
                LOG_LONG_CSV = bool(params.get("HEATMAP_LOG_LONG_CSV", False))            # default False
                LOG_GENERAL_VS_PERSONAL = bool(params.get("HEATMAP_LOG_GVP_CSV", False))  # default False
                LOG_TOP_FEATURES_CSV = bool(params.get("HEATMAP_LOG_TOP_FEATURES", False))# default False
                ...
                sel_node = int(target_node_global_idx)
                sel_row = np.where(all_indices_nz == sel_node)[0]
                sel_row = int(sel_row[0]) if len(sel_row) else None

                for node_id in sampled_nodes.tolist():
                    rr = np.where(all_indices_nz == int(node_id))[0]
                    if len(rr) == 0:
                        continue
                    rr = int(rr[0])

                    attn_row = attention_matrix_nz[rr]
                    pos_list = _select_positions_topk(attn_row, topk=HM_POS_TOPK, min_w=HM_POS_MINW)

                    for explain_pos in pos_list:
                        df_feat_tmp, _, _ = maskopt_e2e_explain(
                            model=model,
                            input_graphs=input_graphs,
                        ...
                        if df_feat_tmp is None or df_feat_tmp.empty:
                            continue

                        idx_col = _pick_col(df_feat_tmp, ["feature_idx","feat_idx","feature_index","idx"])
                        imp_col = _pick_col(df_feat_tmp, ["importance","score_impact","impact","weight","mask","Importance"])
                        if idx_col is None or imp_col is None:
                            continue

                        top = df_feat_tmp.sort_values(imp_col, ascending=False).head(HM_FEAT_TOPK)
                        for _, r in top.iterrows():
                            ...
                            if 0 <= fi < Fdim:
                                ...
                                rows.append(rec)
                                if sel_row is not None and int(node_id) == sel_node:
                                    rows_user.append(rec)

                df_long = pd.DataFrame(rows)

                # ❌容量節約：long CSVはデフォルト保存しない
                if LOG_LONG_CSV and (not df_long.empty):
                    heat_csv = f"agg_feat_heat_long_{run_name}.csv"
                    df_long.to_csv(heat_csv, index=False, float_format="%.8e", encoding="utf-8-sig")
                    mlflow.log_artifact(heat_csv, artifact_path="xai/agg_heatmap")
                    os.remove(heat_csv)

                if df_long.empty:
                    print("⚠️ agg heatmap: df_long empty (no explanations collected)")
                else:
                    # ---- strength: median importance
                    g_med = df_long.groupby(["pos","feature"], as_index=False)["importance"].median()
                    piv_med = g_med.pivot(index="pos", columns="feature", values="importance")
                    piv_med = piv_med.reindex(index=list(range(T))).fillna(0.0)

                    # ---- generality: prevalence (unique users)
                    g_cnt = df_long.drop_duplicates(["node_id","pos","feature"]).groupby(["pos","feature"]).size().reset_index(name="count")
                    g_cnt["prevalence"] = g_cnt["count"] / float(len(sampled_nodes) + 1e-9)
                    piv_prev = g_cnt.pivot(index="pos", columns="feature", values="prevalence")
                    piv_prev = piv_prev.reindex(index=list(range(T))).fillna(0.0)

                    # ---- show top features by prevalence (一般に効く順)
                    overall_prev = piv_prev.mean(axis=0).sort_values(ascending=False)
                    show_feats = overall_prev.head(HM_SHOW_TOPF).index.tolist()

                    # （任意）Top特徴だけ小さいCSVにする（数KB）
                    if LOG_TOP_FEATURES_CSV:
                        df_top = overall_prev.head(HM_SHOW_TOPF).reset_index()
                        df_top.columns = ["feature", "mean_prevalence"]
                        top_csv = f"agg_heat_top_features_{run_name}.csv"
                        df_top.to_csv(top_csv, index=False, float_format="%.8e", encoding="utf-8-sig")
                        mlflow.log_artifact(top_csv, artifact_path="xai/agg_heatmap")
                        os.remove(top_csv)

                    med_mat = piv_med[show_feats].values
                    prev_mat = piv_prev[show_feats].values

                    pos_labels = [f"pos{p}" for p in range(T)]

                    # ✅最終成果物だけ保存（2枚）
                    med_png = f"agg_heat_median_{run_name}.png"
                    _plot_heatmap(
                        med_mat, show_feats, pos_labels,
                        title=f"Aggregate feature strength (median) | sample={len(sampled_nodes)}",
                        out_png=med_png
                    )
                    mlflow.log_artifact(med_png, artifact_path="xai/agg_heatmap")
                    os.remove(med_png)

                    prev_png = f"agg_heat_prevalence_{run_name}.png"
                    _plot_heatmap(
                        prev_mat, show_feats, pos_labels,
                        title=f"Aggregate feature generality (prevalence) | sample={len(sampled_nodes)}",
                        out_png=prev_png, vmax_quantile=0.99
                    )
                    mlflow.log_artifact(prev_png, artifact_path="xai/agg_heatmap")
                    os.remove(prev_png)

                    # ❌容量節約：general_vs_personal もデフォルト保存しない
                    if LOG_GENERAL_VS_PERSONAL and (len(rows_user) > 0):
                        df_u = pd.DataFrame(rows_user)
                        u_med = df_u.groupby(["pos","feature"], as_index=False)["importance"].max()
                        u_piv = u_med.pivot(index="pos", columns="feature", values="importance").reindex(index=list(range(T))).fillna(0.0)
                        ...
                        for p in range(T):
                            for f in show_feats:
                                comp_rows.append({
                                    "pos": p,
                                    "feature": f,
                                    "user_importance": float(u_piv.loc[p, f]) if f in u_piv.columns else 0.0,
                                    "global_median": float(piv_med.loc[p, f]) if f in piv_med.columns else 0.0,
                                    "global_prevalence": float(piv_prev.loc[p, f]) if f in piv_prev.columns else 0.0,
                                })
                        df_comp = pd.DataFrame(comp_rows)
                        df_comp["personal_score"] = df_comp["user_importance"] * (1.0 - df_comp["global_prevalence"])
                        df_comp = df_comp.sort_values("personal_score", ascending=False)

                        comp_csv = f"general_vs_personal_{sel_node}_{run_name}.csv"
                        df_comp.to_csv(comp_csv, index=False, float_format="%.8e", encoding="utf-8-sig")
                        mlflow.log_artifact(comp_csv, artifact_path="xai/agg_heatmap")
                        os.remove(comp_csv)

            except Exception as e:
                print("⚠️ agg heatmap failed:", e)
...
def main():
    ...
    print(f"[Target] influencer_name arg = {args.influencer_name}")

    target_node_idx = None

    # normalize string
    inf_name = args.influencer_name
    if inf_name is not None:
        inf_name = str(inf_name).strip()
        if inf_name.lower() in ("", "none", "null"):
            inf_name = None

    if inf_name is not None:
        # allow @prefix
        cand = [inf_name]
        if inf_name.startswith("@"):
            cand.append(inf_name[1:])
        cand.append(inf_name.lower())
        if inf_name.startswith("@"):
            cand.append(inf_name[1:].lower())
        ...
        if hit is None:
            # 例として "node_to_idxの先頭" を出すと #hashtag が出て紛らわしいので
            # influencer_indices から実在する例を出す
            idx_to_node = {int(v): str(k) for k, v in node_to_idx.items()}
            example_infs = [idx_to_node[int(i)] for i in list(influencer_indices)[:10]]
            raise ValueError(
                f"Unknown influencer_name: {args.influencer_name!r}. "
                f"Example influencer keys: {example_infs}"
            )

        target_node_idx = int(node_to_idx[hit])
        print(f"[Target] resolved influencer_name={hit} -> node_id={target_node_idx}")

    else:
        print("[Target] influencer_name not provided -> auto-select hub influencer in run_experiment")
        target_node_idx = None
    ...
    base_params.update({
    "DO_AGG_HEATMAP": True,
    "HEATMAP_USERNAMES": "diana.stef,troppaseta,elisanencioni",  # or ["diana.stef", "troppaseta"]
    "HEATMAP_LOG_USERNAMES_CHUNKED": False,
    })
