...
def maybe_download_ckpt_from_mlflow(run_id: str, artifact_path: str) -> str:
    ...
    - Tries the given artifact_path first
    - If it fails, auto-discovers likely checkpoint files under the run's artifacts
      (prefers 'model/model_state.pt', then any '*model_state*.pt', then any '.pt' / '.pth').

    This is intentionally robust for cases where you accidentally pass 'model_state.pt'
    while the actual artifact is logged under 'model/model_state.pt'.
    """
    from pathlib import Path
    import mlflow
    ...
    def _try_download(path: str) -> str | None:
        try:
            ...
            print(f"[CKPT] download failed for artifact_path='{path}': {e}")
            return None

    # 1) Try the user-provided path first
    local = _try_download(str(artifact_path))
    if local is not None:
        print(f"[CKPT] downloaded: {artifact_path} -> {local}")
        return local

    # 2) Auto-discover available artifacts
    client = MlflowClient()

    def _walk(prefix: str = ""):
        items = client.list_artifacts(run_id, path=prefix)
        for it in items:
    ...
    all_files = list(_walk(""))
    if not all_files:
        raise RuntimeError(f"No artifacts found for run_id={run_id}. Check MLFLOW_TRACKING_URI / run_id.")

    def _rank(p: str) -> tuple:
        # lower is better
        base = os.path.basename(p)
        if p == "model/model_state.pt":
            return (0, 0, len(p))
        if base == "model_state.pt":
            return (1, 0, len(p))
        if "model_state" in base and base.endswith(".pt"):
            return (2, 0, len(p))
        if base.endswith(".pt") or base.endswith(".pth"):
            return (3, 0, len(p))
        return (9, 0, len(p))

    candidates = [p for p in all_files if p.endswith(".pt") or p.endswith(".pth")]
    if not candidates:
        preview = "\n".join(all_files[:50])
        raise RuntimeError(
            f"No .pt/.pth artifacts found in run_id={run_id}. "
            f"Available artifacts (first 50):\n{preview}"
        )

    candidates = sorted(candidates, key=_rank)
    print("[CKPT] available checkpoint-like artifacts (top 10):")
    for p in candidates[:10]:
        print("  -", p)

    # 3) Try the best candidate
    best = candidates[0]
    local = _try_download(best)
    if local is None:
        raise RuntimeError(
            f"Failed to download any checkpoint artifacts from run_id={run_id}. "
            f"Tried: {artifact_path} then {best}. "
            f"See printed candidate list above and pass --mlflow_ckpt_artifact explicitly."
        )

    print(f"[CKPT] downloaded (auto): {best} -> {local}")
    return local
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        if mode_run != "infer":
            try:
                try:
                    from pathlib import Path
                    ckpt_dir = Path(params.get("CKPT_LOCAL_DIR", "checkpoints"))
                    ckpt_dir.mkdir(parents=True, exist_ok=True)
                    ckpt_local = ckpt_dir / f"{run_name}_model_state.pt"
                    ckpt_local, cfg_local = save_model_checkpoint(
                        model, params, feature_dim=feature_dim, out_path=str(ckpt_local)
                    )
                    ...
                    keep_local = bool(params.get("KEEP_LOCAL_CKPT", True))
                    if keep_local:
                        print(f"[Checkpoint] kept local files: {ckpt_local} , {cfg_local}")
                    else:
                        try:
