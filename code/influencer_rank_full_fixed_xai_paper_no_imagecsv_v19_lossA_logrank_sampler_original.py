...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        if bool(params.get("USE_SAMPLER", False)):
            ...
            # ---- Right/Left tail params (sampler) ----
            RIGHT_Q = float(params.get("RIGHT_Q", 0.90))  # 上位何%からを右tailにするか
            RIGHT_W = float(params.get("RIGHT_W", 5.0))   # 右tailの重み

            LEFT_Q  = float(params.get("LEFT_Q", 0.10))   # 下位何%までを左tailにするか
            LEFT_W  = float(params.get("LEFT_W", 1.0))    # 左tailの重み（1.0なら無効）

            y = train_dataset.tensors[1].detach().cpu().numpy().astype(float)  # target

            thr_hi = np.quantile(y, RIGHT_Q)
            thr_lo = np.quantile(y, LEFT_Q)

            weights = np.ones_like(y, dtype=np.float64)
            weights = np.where(y >= thr_hi, RIGHT_W, weights)  # 右tail
            weights = np.where(y <= thr_lo, LEFT_W,  weights)  # 左tail
        ...
        if mode_run != "infer":
            ...
            for epoch in range(params["EPOCHS"]):
                ...
                for batch in dataloader:
                    ...
                    thr_hi_t = torch.tensor(thr_hi_global, device=device, dtype=b_target.dtype)
                    thr_lo_t = torch.tensor(thr_lo_global, device=device, dtype=b_target.dtype)
...
def main():
    ...
    sweep_grid = {
        ...
        # "RIGHT_Q": [0.90, 0.85, 0.8],
        # "RIGHT_W": [5.0, 8.0, 12, 20.0],
        "LEFT_Q": [0.10, 0.15],
        "LEFT_W": [3.0, 5.0, 12.0, 20.0],
    }
