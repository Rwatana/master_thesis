...
def fit_affine_calibration_log1p(y_true, y_pred, eps=1e-12):
    yt = np.log1p(np.maximum(y_true, 0.0))
    yp = np.log1p(np.maximum(y_pred, 0.0) + eps)
    A = np.vstack([yp, np.ones_like(yp)]).T
    a, b = np.linalg.lstsq(A, yt, rcond=None)[0]
    return float(a), float(b)

def apply_affine_calibration_log1p(y_pred, a, b, eps=1e-12):
    yp = np.log1p(np.maximum(y_pred, 0.0) + eps)
    ycal = np.expm1(a * yp + b)
    return np.maximum(ycal, 0.0)
...
def load_model_from_ckpt(ckpt_path: str, device: torch.device):
    ...
    if isinstance(ckpt, dict) and "state_dict" in ckpt and "feature_dim" in ckpt:
        ...
        if len(missing) > 0 or len(unexpected) > 0:
            print(f"[ckpt] load_state_dict(strict=False) missing={missing} unexpected={unexpected}")
        model.eval()
...
class HardResidualInfluencerModel(nn.Module):
    def __init__(self, feature_dim, gcn_dim, rnn_dim, num_gcn_layers=2, dropout_prob=0.5, projection_dim=128):
        ...
        # ===== D-1: learnable calibration (needs re-training)
        self.calib_a_log = nn.Parameter(torch.tensor(0.0))  # exp()して正
        self.calib_b     = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, gcn_embeddings, raw_features, baseline_scores=None):
        ...
        calib_a = torch.exp(self.calib_a_log).clamp(1e-6, 1e6)
        raw_output = calib_a * raw_output + self.calib_b
...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        with torch.no_grad():
            ...
            # ===== D-2: post-hoc calibration (no re-training)
            if bool(params.get("POSTHOC_CALIB", False)):
                a, b = fit_affine_calibration_log1p(true_scores, predicted_scores)
                predicted_scores = apply_affine_calibration_log1p(predicted_scores, a, b)
                print(f"[posthoc_calib] a={a:.6f}, b={b:.6f}")
...
def main():
    ...
    sweep_grid = {
        ...
        'POINTWISE_LOSS_WEIGHT': [0.01, 0.1, 0.3, 0.5],
        # 'GCN_DIM': [64, 128],
        # 'RNN_DIM': [64, 128],
