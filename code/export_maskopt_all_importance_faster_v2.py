...
class E2EMaskOptWrapper(nn.Module):
    ...
    def __init__(
        ...
        pre_cached_proj=None,
        pre_cached_gcn=None,
        cache_hops: int = -1,
        max_incident_edges: int = -1,
        seed: int = 42,
    ):
        super().__init__()
        self.model = model
        ...
        # Cache settings (avoid moving full graphs to device)
        try:
            num_gcn_layers = len(self.model.gcn_encoder.convs)
        except Exception:
            num_gcn_layers = 2
        self.cache_hops = int(cache_hops) if (cache_hops is not None and int(cache_hops) > 0) else max(int(self.num_hops), int(num_gcn_layers))
        self.max_incident_edges = int(max_incident_edges) if (max_incident_edges is not None) else -1
        self.seed = int(seed) if (seed is not None) else 42

        # Allow external cache reuse (compute once per target, reuse across explain_pos)
        if pre_cached_proj is not None and pre_cached_gcn is not None:
            if len(pre_cached_proj) != self.T or len(pre_cached_gcn) != self.T:
                raise ValueError("pre_cached_proj/pre_cached_gcn length must match T")
            self.cached_proj = [p.to(self.device) if (p is not None) else None for p in pre_cached_proj]
            self.cached_gcn  = [z.to(self.device) if (z is not None) else None for z in pre_cached_gcn]
        else:
            self._prepare_cache()
    ...
    def _prepare_cache(self):
        """
        Prepare per-month cached vectors for months other than explain_pos.

        IMPORTANT:
          - Never move the full monthly graph (millions of nodes) onto device.
          - Extract a small k-hop subgraph on CPU first, then move only the subgraph.
        """
        self.model.eval()

        def _extract_khop_subgraph_cpu(g, hops: int):
            x_full = g.x
            ei_full = g.edge_index
            if (not self.use_subgraph) or (ei_full is None) or (ei_full.numel() == 0):
                # This path can be extremely slow / memory heavy for large graphs.
                x = x_full.to(self.device)
                ei = ei_full.to(self.device) if ei_full is not None else torch.empty((2, 0), dtype=torch.long, device=self.device)
                return x, ei, int(self.target_global), None
            ...
            return x_sub.to(self.device), ei_sub.to(self.device), int(mapping.item()), subset

        hops = int(getattr(self, "cache_hops", self.num_hops))
        for t, g in enumerate(self.input_graphs):
            if t == self.explain_pos:
                continue
            x_sub, ei_sub, tgt_local, _ = _extract_khop_subgraph_cpu(g, hops)
            p = self.model.projection_layer(x_sub)
            out = _gcn_forward_concat(self.model.gcn_encoder, p, ei_sub, edge_weight=None)
            self.cached_proj[t] = p[tgt_local].detach()
            self.cached_gcn[t] = out[tgt_local].detach()
    def _prepare_explain_graph(self):
        """
        Build the (small) explain graph for explain_pos.

        IMPORTANT:
          - Do NOT do `g.to(device)` on the full monthly graph.
          - Extract subgraph on CPU, then move only the subgraph tensors to device.
          - Optionally downsample incident edges to speed up MaskOpt on very high-degree targets.
        """
        g = self.input_graphs[self.explain_pos]  # keep on CPU
        x_full = g.x
        ei_full = g.edge_index

        if (not self.use_subgraph) or (ei_full is None) or (ei_full.numel() == 0):
            self.x_exp = x_full.to(self.device)
            self.ei_exp = ei_full.to(self.device) if ei_full is not None else torch.empty((2, 0), dtype=torch.long, device=self.device)
            self.target_local = int(self.target_global)
            ...
            # Optional: prune to at most max_incident_edges incident edges (reduces GCN cost)
            if getattr(self, "max_incident_edges", -1) is not None and int(self.max_incident_edges) > 0 and ei_sub.numel() > 0:
                src, dst = ei_sub
                incident = (src == target_local) | (dst == target_local)
                inc_idx = torch.where(incident)[0]

                if inc_idx.numel() > int(self.max_incident_edges):
                    gen = torch.Generator()
                    seed = int(getattr(self, "seed", 42))
                    seed = (seed + int(self.target_global) * 1315423911 + int(self.explain_pos) * 2654435761) & 0xFFFFFFFF
                    gen.manual_seed(seed)
                    perm = torch.randperm(inc_idx.numel(), generator=gen)[: int(self.max_incident_edges)]
                    keep_inc = inc_idx[perm]
                else:
                    keep_inc = inc_idx

                nbr = torch.where(src[keep_inc] == target_local, dst[keep_inc], src[keep_inc])
                keep_nodes = torch.unique(torch.cat([torch.tensor([target_local], dtype=torch.long), nbr]))
                keep_nodes = torch.cat([torch.tensor([target_local], dtype=torch.long), keep_nodes[keep_nodes != target_local]])

                new_id = torch.full((x_sub.size(0),), -1, dtype=torch.long)
                new_id[keep_nodes] = torch.arange(keep_nodes.numel(), dtype=torch.long)

                mask_e = torch.zeros((ei_sub.size(1),), dtype=torch.bool)
                mask_e[keep_inc] = True
                ei_pruned = ei_sub[:, mask_e]
                ei_pruned = new_id[ei_pruned]

                loop = torch.arange(keep_nodes.numel(), dtype=torch.long)
                loops = torch.stack([loop, loop], dim=0)
                ei_pruned = torch.cat([ei_pruned, loops], dim=1)

                if self.undirected and ei_pruned.numel() > 0:
                    ei_pruned = to_undirected(ei_pruned, num_nodes=keep_nodes.numel())
                    ei_pruned = coalesce(ei_pruned, num_nodes=keep_nodes.numel())

                x_sub = x_sub[keep_nodes]
                subset = subset[keep_nodes]
                ei_sub = ei_pruned
                target_local = 0

            self.x_exp = x_sub.to(self.device)
            self.ei_exp = ei_sub.to(self.device)
            self.target_local = int(target_local)
            self.local2global = subset

        src, dst = self.ei_exp
        incident = (src == self.target_local) | (dst == self.target_local)
        self.incident_edge_idx = torch.where(incident)[0].to(self.device)

        if self.edge_grouping == "neighbor":
            src_i = src[self.incident_edge_idx]
            dst_i = dst[self.incident_edge_idx]
            nbr = torch.where(src_i == self.target_local, dst_i, src_i)
            uniq_nbr, inv = torch.unique(nbr, return_inverse=True)
            self.edge_gid = inv

            names = []
            edge_group_global = []
            for u in uniq_nbr.detach().cpu().tolist():
                if self.local2global is not None:
                    gid = int(self.local2global[u].item())
                else:
                    gid = int(u)
                edge_group_global.append(gid)
                names.append(f"nbr_{gid}")

            self.edge_group_names = names
            self.edge_group_global = edge_group_global
            self.edge_group_sizes = torch.bincount(inv, minlength=int(uniq_nbr.numel())).detach().cpu().tolist()

            self.edge_params = int(uniq_nbr.numel())
        else:
            self.edge_gid = None
            self.edge_group_names = None
            self.edge_group_global = None
            self.edge_group_sizes = None
            self.edge_params = int(self.incident_edge_idx.numel())
...
def export_all_elements(
    ...
    topk_feat_impact: int = -1,
    topk_node_impact: int = -1,
    ...
    feat_indices = [j for j in range(Fdim) if float(feat_np[j]) >= float(min_importance)]
    feat_indices.sort(key=lambda j: float(feat_np[j]), reverse=True)
    if topk_feat_impact is not None and int(topk_feat_impact) > 0:
        feat_indices = feat_indices[: int(topk_feat_impact)]
    rows_feat = []
    ...
    edge_indices = [g for g in range(Edim) if float(edge_np[g]) >= float(min_importance)]
    edge_indices.sort(key=lambda g: float(edge_np[g]), reverse=True)
    if topk_node_impact is not None and int(topk_node_impact) > 0:
        edge_indices = edge_indices[: int(topk_node_impact)]
...
def run_export_for_targets(
    ...
    # Compute reusable per-target caches once (for all months), and reuse across explain_pos.
    # This avoids recomputing (T-1) cached embeddings for each explain_pos.
    cache_hops = int(args.cache_hops) if hasattr(args, "cache_hops") and int(args.cache_hops) > 0 else max(int(args.num_hops), int(args.num_gcn_layers))

    @torch.inference_mode()
    def _compute_target_cache(tgt_global: int):
        proj_vecs = [None] * len(input_graphs)
        gcn_vecs = [None] * len(input_graphs)
        for t, g in enumerate(input_graphs):
            # Extract k-hop subgraph on CPU first
            subset, ei_sub, mapping, _ = k_hop_subgraph(
                int(tgt_global),
                int(cache_hops),
                g.edge_index,
            ...
            x_sub = g.x[subset]
            if args.undirected and ei_sub.numel() > 0:
                ei_sub = to_undirected(ei_sub, num_nodes=x_sub.size(0))
                ei_sub = coalesce(ei_sub, num_nodes=x_sub.size(0))
            ...
            p = model.projection_layer(x_sub)
            out = _gcn_forward_concat(model.gcn_encoder, p, ei_sub, edge_weight=None)
            tgt_local = int(mapping.item())
            proj_vecs[t] = p[tgt_local].detach()
            gcn_vecs[t] = out[tgt_local].detach()
        return proj_vecs, gcn_vecs
    ...
    for tgt in tqdm(targets, desc=f"[{str(device)}] targets", leave=False):
        pre_cached_proj, pre_cached_gcn = _compute_target_cache(int(tgt))
        ...
        for pos in explain_positions:
            wrapper = E2EMaskOptWrapper(
                ...
                pre_cached_proj=pre_cached_proj,
                pre_cached_gcn=pre_cached_gcn,
                cache_hops=cache_hops,
                max_incident_edges=int(args.max_incident_edges) if hasattr(args, 'max_incident_edges') else -1,
                seed=int(args.seed),
            )
            ...
            df_feat, df_node, meta = export_all_elements(
                ...
                topk_feat_impact=int(args.topk_feat_impact),
                topk_node_impact=int(args.topk_node_impact),
            )
...
def parse_args():
    ...
    p.add_argument("--directed", action="store_true", help="Do not symmetrize subgraph edges (keep directed).")
    ...
    p.add_argument("--topk-feat-impact", type=int, default=-1,
                   help="If >0, compute score_impact only for top-K features by importance (speed).")
    p.add_argument("--topk-node-impact", type=int, default=-1,
                   help="If >0, compute score_impact only for top-K edge-groups/nodes by importance (speed).")
    p.add_argument("--max-incident-edges", type=int, default=-1,
                   help="If >0, prune explain subgraph to at most this many incident edges (speed).")
    p.add_argument("--cache-hops", type=int, default=-1,
                   help="If >0, hops used for cache embeddings (default=max(num_hops,num_gcn_layers)).")
    ...
    args = p.parse_args()
    args.undirected = (not getattr(args, 'directed', False))
    return args
