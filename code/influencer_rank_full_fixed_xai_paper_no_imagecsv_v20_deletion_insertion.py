...
def run_experiment(params, graphs_data, target_node_idx, experiment_id=None):
    ...
    def deletion_insertion_feature_v3(
        ...
        target_node,
        baseline_score,
        ranked_feature_order,   # ranked list (subset OK)
        mode="deletion",
        random_trials=50,
        seed=0,
        device="cpu",
        ...
        full_order = ranked + rest            # ★端点を揃えるキモ
        ...
        for k in k_list:
            if mode == "deletion":
                ...
                masked = full_order[k:]   # keep top-k, mask rest
            else:
                raise ValueError(mode)
        ...
        # endpoint checks（ここが通れば “定義は正しい”）
        # deletion: k=0==orig, k=F==baseline-like
        # insertion: k=F==orig, k=0==baseline-like
