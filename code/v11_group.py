...
# -*- coding: utf-8 -*-
"""
InfluencerRank reproduction + training/inference + XAI (MaskOpt E2E) — FULL SCRIPT (fixed)

Fixes:
  - Define T and input_graphs before using them in the explanation block
  - Add x_override support to E2EMaskOptWrapper.predict_with_gates (for feature ablation)
  - Define ones_feat in feature ablation block
  - Remove duplicate edge_rows.append block
  - Define tag in run_maskopt_for_all_months

Usage example:
  python influencer_rank_full_fixed.py --visible 0 --device 0
...
def restore_edge_group_names(df_edge, wrapper, node_to_idx=None):
    ...
    復元ロジック（順に試す）:
      ...
      B) それ以外なら XXX を「global node id」とみなして名前へ
      C) wrapper 側に group->neighbor の対応表があればそれを使用
      D) どれも無理なら元のまま
    """
    import pandas as pd
    ...
    # 逆引き: global_idx -> node_name
    idx_to_node = {v: k for k, v in node_to_idx.items()} if node_to_idx is not None else None

    def _global_name(global_idx: int) -> str:
        if idx_to_node is None:
            return f"Node_{int(global_idx)}"
        return idx_to_node.get(int(global_idx), f"Node_{int(global_idx)}")

    # 代表エッジから「相手ノード」を復元
    ei = getattr(wrapper, "ei_exp", None)
    local2global = getattr(wrapper, "local2global", None)
    target_local = int(getattr(wrapper, "target_local", -1))

    E = int(ei.size(1)) if ei is not None else 0

    # wrapper が group のキー一覧を持っている場合（あれば使う）
    group_keys = getattr(wrapper, "edge_group_keys", None)  # Optional: list-like

    def _try_from_edge_index(edge_idx: int) -> str | None:
        if ei is None or edge_idx < 0 or edge_idx >= E or target_local < 0:
            return None
        u = int(ei[0, edge_idx].item())
        v = int(ei[1, edge_idx].item())
        nb_local = v if u == target_local else u
        nb_global = int(local2global[nb_local].item()) if local2global is not None else int(nb_local)
        return _global_name(nb_global)

    def _try_from_group_key(gid: int) -> str | None:
        # まずは gid を global node id として解釈
        if gid >= 0:
            name = _global_name(gid)
        ...
        if group_keys is not None:
            try:
                if 0 <= gid < len(group_keys):
                    key = group_keys[gid]
                    # key が global node id なら名前へ
                    if isinstance(key, int):
                        return _global_name(int(key))
                    # key が文字列（=実名）ならそのまま
    ...
    def _restore(name_str: str) -> str:
        s = str(name_str)
        if not s.startswith("edge_group_"):
            return s

        key = s[len("edge_group_"):]
        try:
            gid = int(key)
        except Exception:
            return s
        ...
        # A) edge index として復元
        nm = _try_from_edge_index(gid)
        if nm is not None:
            return nm

        # B/C) global id or wrapper mapping
        nm = _try_from_group_key(gid)
        if nm is not None:
            return nm

        return s

    df_edge["Name"] = df_edge["Name_raw"].map(_restore)
    return df_edge
...
def setup_mlflow_experiment(
    ...
    """
    Local-first MLflow setup (A案: ローカル完結 / これまで通り).
    ...
    Why this exists:
      ...
      - When you later switch to a file-based tracking URI (file:///...), MLflow cannot
        resolve `mlflow-artifacts:` without an HTTP tracking server, and `mlflow.log_artifact`
        will crash.

    This helper:
      1) Sets tracking URI (env MLFLOW_TRACKING_URI is respected unless overridden)
      2) Ensures the chosen experiment uses *file:* artifact_location when tracking is file:*
         (creates a new experiment with a suffix if needed)
      3) Returns (experiment_name, experiment_id) so we can pass experiment_id to start_run()
         and avoid accidentally logging into the wrong (old) experiment.
    """
    import mlflow
    import datetime
    ...
    # For local (file store), we want file:// artifacts. We'll create a new experiment if needed.
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

    # Decide artifact root for local runs (only used when creating a new experiment locally)
    cwd = Path.cwd()
    ...
    if exp is None:
        try:
            ...
            # Race / already created: fall back to lookup
    ...
    # Make it explicit for downstream code
...
def plot_attention_weights(attention_matrix, run_name):
    ...
    # --- Numeric artifacts (for debugging 0.0 vs tiny values) ---
    labels = [f"T-{len(mean_att)-1-i}" for i in range(len(mean_att))]
    labels[-1] = "Current (T)"
...
class E2EMaskOptWrapper(nn.Module):
    ...
    def _apply_feature_gate(self, x, feat_gate):
        ...
        # "target" only: avoid in-place update on a view (can break autograd)
        n = x.size(0)
        sel = F.one_hot(
        ...
        # x + sel * x * (gate - 1): only target row is gated
    ...
    def _make_edge_weight(self, edge_gate):
        ...
        if self.edge_mask_scope == "incident":
            # Direct assignment (do not read w[idx] on RHS)
            w = w.clone()
...
#     if coeffs is None:
#         coeffs = {"edge_size": 0.01, "edge_ent": 0.0, "node_feat_size": 0.01, "node_feat_ent": 0.0}
...
#         edge_gate = torch.sigmoid(edge_logits) if Edim > 0 else None
...
#         # Optional: contrastive term (complementary mask should CHANGE the prediction)
#         if use_contrastive:
#             feat_gate_drop = (1.0 - feat_gate).clamp(0.0, 1.0)
#             if Edim > 0 and edge_gate is not None:
#                 edge_gate_drop = (1.0 - edge_gate).clamp(0.0, 1.0)
#             else:
#                 edge_gate_drop = None
...
#             best["edge"] = (edge_gate.detach().clone() if Edim > 0 else None)
...
#             print(f"  [MaskOpt] ep={ep:4d} loss={lval:.6e} fid={float(loss_fid.item()):.3e} drop_d={float(delta.item()):.3e} pred={float(pred.item()):.6f} "
#                   f"feat_max={feat_max:.4f} edge_max={edge_max:.4f}")
...
#     feat_stats = _mask_stats(feat_gate)
#     edge_stats = _mask_stats(edge_gate)

#     fid_err = abs(float(best["pred"]) - float(orig)) if best.get("pred") is not None else float("inf")
#     flat_feat = (feat_stats["std"] < 1e-3) or ((feat_stats["max"] - feat_stats["mean"]) < 1e-3)
#     flat_edge = (edge_stats["std"] < 1e-3) or ((edge_stats["max"] - edge_stats["mean"]) < 1e-3)

#     # Heuristic: explainable if masks are not flat AND fidelity is decent.
#     explainable = (fid_err <= float(0.01)) and (not (flat_feat and flat_edge))
#     explain_skip_reason = None
#     if not explainable:
#         explain_skip_reason = (
#             f"flat_mask_or_low_fidelity: fid_err={fid_err:.3e} "
#             f"feat_std={feat_stats['std']:.3e} edge_std={edge_stats['std']:.3e}"
#         )
#     # ---------------- Feature ablation sign ----------------
#     feat_rows = []
...
#         ones_feat = torch.ones(Fdim, device=device)  # FIX
#         ones_edge = (torch.ones(Edim, device=device) if Edim > 0 else None)

#         with torch.no_grad():
#             with cudnn_ctx:
#                 pred_full_unmasked = float(wrapper.predict_with_gates(ones_feat, ones_edge).item())

#         # x0 = wrapper.x_exp.detach()
#         # base = _feature_baseline(x0, node_mask=None, method="mean")
        
#         full_x = wrapper.input_graphs[wrapper.explain_pos].x.to(device)
#         base = full_x.mean(dim=0)  # 月の全ノード平均
#         x0 = wrapper.x_exp.detach()
...
#                 continue
#             x_ab = x0.clone()
#             tj = x0[wrapper.target_local, j]
#             x_ab[wrapper.target_local, j] = (1.0 - ablate_strength) * tj + ablate_strength * base[j]

#             with torch.no_grad():
#                 with cudnn_ctx:
#                     pred_abl = float(wrapper.predict_with_gates(ones_feat, ones_edge, x_override=x_ab).item())
...
#                 "Name": name,
#                 "Importance": imp,
#                 "Score_Impact(orig-ablated)": float(diff),
#                 "Direction": direction
#             })
...
#     if Edim > 0 and (edge_gate is not None) and (edge_gate.numel() > 0):
#         ei = wrapper.ei_exp.detach().cpu()
#         src = ei[0].numpy()
#         dst = ei[1].numpy()

#         idx_to_node = {v: k for k, v in node_to_idx.items()} if node_to_idx is not None else None
#         def _global_name(global_idx):
#             if idx_to_node is None:
#                 return f"Node_{int(global_idx)}"
#             return idx_to_node.get(int(global_idx), f"Node_{int(global_idx)}")

#         target_local = wrapper.target_local

#         if edge_mask_scope == "incident":
#             full_edge_imp = np.zeros(ei.size(1), dtype=np.float32)
#             inc = wrapper.incident_edge_idx.detach().cpu().numpy()
#             full_edge_imp[inc] = edge_gate.detach().cpu().numpy()
#         else:
#             full_edge_imp = edge_gate.detach().cpu().numpy()

#         incident_edges = np.where((src == target_local) | (dst == target_local))[0]
#         neighbor_to_best = {}
#         for e_idx in incident_edges:
#             imp = float(full_edge_imp[e_idx])
#             if imp < min_show:
#                 continue
#             u = int(src[e_idx]); v = int(dst[e_idx])
#             nb_local = v if u == target_local else u
#             nb_global = int(wrapper.local2global[nb_local].item()) if wrapper.local2global is not None else nb_local
#             nb_name = _global_name(nb_global)
#             if (nb_name not in neighbor_to_best) or (imp > neighbor_to_best[nb_name]["Importance"]):
#                 neighbor_to_best[nb_name] = {
#                     "Name": nb_name,
#                     "Importance": imp,
#                     "edge_local_index": int(e_idx),
#                 }

#         sorted_edges = sorted(neighbor_to_best.values(), key=lambda d: d["Importance"], reverse=True)[:topk_edge]

#         with torch.no_grad():
#             with cudnn_ctx:
#                 # Use unmasked baseline for effect size
#                 ones_feat = torch.ones(Fdim, device=device)
#                 ones_edge = (torch.ones(Edim, device=device) if Edim > 0 else None)
#                 pred_full = float(wrapper.predict_with_gates(ones_feat, ones_edge).item())

#         for item in sorted_edges:
#             e_local = item["edge_local_index"]
#             E = int(wrapper.ei_exp.size(1))
#             if E == 0:
#                 continue

#             edge_w_ab = torch.ones(E, device=device, dtype=torch.float32)
#             u = int(wrapper.ei_exp[0, e_local].item())
#             v = int(wrapper.ei_exp[1, e_local].item())
#             uu = wrapper.ei_exp[0]
#             vv = wrapper.ei_exp[1]
#             both_dir = ((uu == u) & (vv == v)) | ((uu == v) & (vv == u))

#             strength = 1.0
#             edge_w_ab[both_dir] = (1.0 - strength) * edge_w_ab[both_dir] + strength * 0.0

#             with torch.no_grad():
#                 with cudnn_ctx:
#                     pred_abl = float(wrapper.predict_with_gates(
#                         ones_feat, ones_edge, edge_weight_override=edge_w_ab
#                     ).item())
...
#             # Persist in scientific notation so tiny impacts don't look like literal zeros
...
#         "explain_skip_reason": explain_skip_reason
#     }
...
#     contrastive_weight=1.0,
#     tag="pos_0",
#     # ======= NEW: baseline/ablation controls =======
#     baseline_scope="full_graph_month",   # "full_graph_month" | "explain_subgraph" | "target_only"
#     baseline_method="mean",              # "mean" | "median"
#     ablate_strength=1.0,                 # 1.0 = fully replace with baseline
#     # ======= NEW: direction (Zero判定) thresholds =======
#     eps_abs_feat=1e-9,
#     eps_rel_feat=1e-6,
#     eps_abs_edge=1e-9,
#     eps_rel_edge=1e-6,
# ):
#     """
#     End-to-end Mask Optimization explainer for a temporal (GCN->RNN->Attn->MLP) pipeline.

#     baseline_scope:
#       - "full_graph_month": baseline = mean/median over ALL nodes in the explain_pos month graph
#       - "explain_subgraph": baseline = mean/median over nodes in the extracted subgraph
#       - "target_only": baseline = 0 vector (or you can customize below)

#     eps_*:
#       Zero判定を abs(diff) <= max(eps_abs, eps_rel*|pred_full|) で行う。
#     """
#     assert len(input_graphs) >= 2, "input_graphs length must be >= 2"
#     if device is None:
#         device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
#     # NOTE: 外側の params(dict) と衝突しやすいので mask_params に変更
...
#             best["loss"] = lval
#             best["feat"] = feat_gate.detach().clone()
#             best["edge"] = (edge_gate.detach().clone() if (Edim > 0 and edge_gate is not None) else None)
#             best["pred"] = float(pred.detach().item())
...
#         # x0: explain subgraph feature matrix
#         x0 = wrapper.x_exp.detach()

#         if baseline_scope == "full_graph_month":
#             full_x = wrapper.input_graphs[wrapper.explain_pos].x.to(device)
#             if baseline_method == "median":
#                 base = full_x.median(dim=0).values
#             else:
#                 base = full_x.mean(dim=0)
#         elif baseline_scope == "explain_subgraph":
#             if baseline_method == "median":
#                 base = x0.median(dim=0).values
#             else:
#                 base = x0.mean(dim=0)
#         elif baseline_scope == "target_only":
#             base = torch.zeros(Fdim, device=device, dtype=x0.dtype)
#         else:
#             raise ValueError(f"Unknown baseline_scope={baseline_scope}")
...
#             x_ab = x0.clone()
#             tj = x0[wrapper.target_local, j]
#             # ablate_strength=1.0 => fully baseline
#             x_ab[wrapper.target_local, j] = (1.0 - float(ablate_strength)) * tj + float(ablate_strength) * base[j]
...
#             thr = max(float(eps_abs_feat), float(eps_rel_feat) * abs(float(pred_full_unmasked)))
#             if abs(diff) <= thr:
#                 direction = "Zero (0)"
#             elif diff > 0:
#                 direction = "Positive (+)"
...
#             # drop both directions (u->v and v->u)
#             u = int(wrapper.ei_exp[0, e_local].item())
#             v = int(wrapper.ei_exp[1, e_local].item())
#             uu = wrapper.ei_exp[0]
#             vv = wrapper.ei_exp[1]
...
#             edge_w_ab[both_dir] = 0.0

#             with torch.no_grad():
#                 with cudnn_ctx:
#                     pred_abl = float(wrapper.predict_with_gates(
#                         torch.ones(Fdim, device=device),
#                         torch.ones(Edim, device=device) if Edim > 0 else None,
#                         edge_weight_override=edge_w_ab
#                     ).item())

#             diff = pred_full - pred_abl

#             thr = max(float(eps_abs_edge), float(eps_rel_edge) * abs(float(pred_full)))
#             if abs(diff) <= thr:
#                 direction = "Zero (0)"
#             elif diff > 0:
#                 direction = "Positive (+)"
...
#         "ablate_strength": float(ablate_strength),
...
def maskopt_e2e_explain(
    ...
    # ---- wrapper (edge_grouping対応が無い場合も落ちないように) ----
    try:
        wrapper = E2EMaskOptWrapper(
            model=model,
            input_graphs=input_graphs,
    ...
    print(f"🧠 [MaskOpt] target_node={int(target_node_idx)} explain_pos={explain_pos}/{T-1} orig={orig:.6f}")
    print(f"   use_subgraph={use_subgraph}, num_hops={num_hops}, undirected={undirected}, feat_dim={Fdim}, edge_params={Edim}")
    if hasattr(wrapper, "edge_grouping"):
        try:
            print(f"   edge_grouping={wrapper.edge_grouping}")
        except Exception:
            pass
    ...
    for ep in range(1, epochs + 1):
        ...
        if (ep == 1) or (ep % print_every == 0) or (ep == epochs):
            ...
            print(
                f"    terms: fid*W={float(fid_weight)*float(loss_fid.item()):.3e}, "
                f"contr={float(loss_contrast.item()):.3e}, budget={float(loss_budget.item()):.3e}, "
                f"feat_size={float(loss_feat_size.item()):.3e}, edge_size={float(loss_edge_size.item()):.3e}"
            )
    ...
    # compute base preds for unmasked/masked
    with torch.no_grad():
        with cudnn_ctx:
    ...
    if edge_gate is not None and edge_gate.numel() > 0:
        ...
        # group name resolver（wrapperが持ってたらそれを使う）
        group_names = None
        if hasattr(wrapper, "edge_group_names"):
            try:
                if len(wrapper.edge_group_names) == Edim:
                    group_names = list(wrapper.edge_group_names)
            except Exception:
                pass

        # fallback: neighbor groupingっぽいときだけ推定
        if group_names is None:
            group_names = [f"edge_group_{i}" for i in range(Edim)]

        for g in top_edge_idx:
            imp = float(edge_np[g])
            if imp < min_show:
                continue
        ...
        df_edge = restore_edge_group_names(df_edge, wrapper, node_to_idx=node_to_idx)
...
def run_maskopt_for_all_months(
    ...
    model, graphs, target_node_idx=916490, explain_pos=1,
    feature_names=feature_names, node_to_idx=node_to_idx,
    use_subgraph=True, num_hops=1, edge_mask_scope="incident",
    ...
    impact_reference="masked",   # ← ここ重要（ゼロ地獄を避ける）
    use_contrastive=False,       # まずは切って安定化
)

            if df_feat is not None and not df_feat.empty:
                df_feat.insert(0, "explain_pos", explain_pos)
                df_feat.insert(1, "month_label", label)
                all_feat.append(df_feat)
            if df_edge is not None and not df_edge.empty:
                df_edge.insert(0, "explain_pos", explain_pos)
                df_edge.insert(1, "month_label", label)
...
def compute_time_step_sensitivity(
    ...
    We build the temporal embedding sequence for the target node and estimate each
    time step's contribution by *dropping* (zeroing) that step's embeddings.
    This is cheap, stable, and sufficient to select "効く月" for explanation.

    Returns
    -------
    sens_df : pd.DataFrame
        columns: pos, alpha, pred_full, pred_drop, delta_total, score
...
def mlflow_log_pred_scatter(
    ...
    - step: if not None, artifacts go under f"{artifact_path}/step_{step}"
    """
    import os
    import numpy as np
...
def mlflow_log_maskopt_plots(
    ...
    Expected df columns:
      - df_feat: ["Name","Importance", ...]
      - df_edge: ["Name","Importance", ...]
    """
    import os
...
def run_experiment(params, graphs_data, experiment_id=None):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id):
        ...
        for epoch in range(params['EPOCHS']):
            ...
            # We computed full_seq/full_raw ONCE, so they share one autograd graph.
            # Don't call backward() per-batch; accumulate and backward once per epoch.
            loss_sum = None
            num_batches = 0
        ...
        target_graph = monthly_graphs[-2]  # 11月
        ...
        # --- ALWAYS define these before try ---
        sens_df = None
        sens_selected = None
        ...
        try:
            ...
            # --- export attention alpha + (optional) sensitivity table ---
            ...
            for explain_pos in positions_to_explain:
                ...
                df_feat, df_edge, meta = maskopt_e2e_explain(
                    model, input_graphs, target_node_idx=916490, explain_pos=1,
                    feature_names=feature_names, node_to_idx=node_to_idx,
                    use_subgraph=True, num_hops=1, edge_mask_scope="incident",
                    edge_grouping="neighbor",
                    fid_weight=2000.0,
        ...
        # 例: eval時に集めた y_true_list, y_pred_list がある前提
        mlflow_log_pred_scatter(
            y_true=y_true_list,
            y_pred=y_pred_list,
            tag="test_dec2017",
            step=epoch,              # epochが無ければ None でOK
            artifact_path="plots",
        )
        ...
        mlflow_log_maskopt_plots(
            ...
            topk_edge=15,
            artifact_path="xai",
            fname_prefix=f"node_{target_node_idx}",
        )
...
def main():
    ...
    # Ensure MLflow experiment is compatible with local file store (avoids mlflow-artifacts:// vs file:// mismatch)
    ...
    # --- Base params (shared across runs) ---
    base_params = {
        'name_prefix': 'Run',
        'note': 'Default',
        'LR': 0.003,
    ...
    # Option A (recommended): explicit list of overrides (run in this order).
    #   → ここに「いじりたいパラメータだけ」を dict で列挙してください。
    params_list = [
        # {'LR': 0.003, 'DROPOUT_PROB': 0.2, 'POINTWISE_LOSS_WEIGHT': 0.5},
        # {'LR': 0.001, 'DROPOUT_PROB': 0.2, 'POINTWISE_LOSS_WEIGHT': 0.5},
    ]

    # Option B: grid search (cartesian product). If params_list is empty, this is used.
    sweep_grid = {
        # 'LR': [0.003, 0.001, 0.0003],
        # 'DROPOUT_PROB': [0.1, 0.2, 0.3],
        # 'POINTWISE_LOSS_WEIGHT': [0.0, 0.5, 1.0],
        # 'GCN_DIM': [64, 128],
        # 'RNN_DIM': [64, 128],
    }
    ...
    # Build full sweep list: base_params + overrides
    if len(params_list) > 0:
        overrides_list = list(params_list)  # list of dict overrides (user-defined order)
    else:
        overrides_list = _expand_grid(sweep_grid)  # cartesian product

    params_list = [_merge(base_params, ov) for ov in overrides_list]

    # Run sweep sequentially and keep a local summary
    summary_rows = []
    for i, p in enumerate(params_list):
        p = dict(p)  # safe copy

        # For naming: include only the override dict (what you swept)
        ...
        # Log the key hyperparams (edit freely)
        for k in [
    ...
    # Write + log sweep summary (handy for comparing runs in one place)
    if len(summary_rows) > 1:
        ...
        try:
            with mlflow.start_run(run_name=f"SweepSummary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                ...
                # Log best-of metrics as convenience
                if "mae" in df_sum.columns:
                    best = df_sum.sort_values("mae", ascending=True).iloc[0]
                    mlflow.log_metric("best_mae", float(best["mae"]))
                if "rmse" in df_sum.columns:
                    best = df_sum.sort_values("rmse", ascending=True).iloc[0]
                    mlflow.log_metric("best_rmse", float(best["rmse"]))
                if "pearson_corr" in df_sum.columns:
                    best = df_sum.sort_values("pearson_corr", ascending=False).iloc[0]
                    mlflow.log_metric("best_pearson_corr", float(best["pearson_corr"]))
                if "spearman_corr" in df_sum.columns:
                    best = df_sum.sort_values("spearman_corr", ascending=False).iloc[0]
                    mlflow.log_metric("best_spearman_corr", float(best["spearman_corr"]))

                if "RMSE" in df_sum.columns:
                    best = df_sum.sort_values("RMSE", ascending=True).iloc[0]
                    mlflow.log_metric("best_RMSE", float(best["RMSE"]))
                if "Pearson" in df_sum.columns:
                    best = df_sum.sort_values("Pearson", ascending=False).iloc[0]
                    mlflow.log_metric("best_Pearson", float(best["Pearson"]))
                if "Spearman" in df_sum.columns:
                    best = df_sum.sort_values("Spearman", ascending=False).iloc[0]
                    mlflow.log_metric("best_Spearman", float(best["Spearman"]))
        except Exception as e:
