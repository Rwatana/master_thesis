...
\]
とする．ここで$V_t$はノード集合（ユーザー，タグ，オブジェクト等），$E_t$はエッジ集合（例：ユーザー–タグ，ユーザー–メンションなど），$\phi$はノード/エッジのタイプを表す．$X_t$はノード特徴行列であり，数値特徴（投稿数・投稿間隔・キャプションの長さ etc）に加え，画像・テキストから抽出した特徴（例：画像輝度やテキスト長）など，マルチモーダル特徴を含む．
...
\paragraph{(1) 予測精度（定量）}
解析するにあたって，Instagramデータセット（約10万ユーザー，2017年1月〜12月）を用い，過去11ヶ月を学習用，12月をテスト用とした．評価指標はNDCG@50およびRBP(0.95)を採用し，単純ベースライン（フォロワー数、投稿頻度）に基づく順位付け）と比較した．
...
\begin{table}[t]
  ...
  \begin{tabular}{l|ccc}
    \hline
    手法 & NDCG@50 & RBP(0.95) & Pearson \\
    \hline
    InfluencerRank & \textbf{0.89} & \textbf{0.043} & \textbf{0.48} \\
    UA(投稿頻度) & 0.49 & 0.024 & 0.09 \\
    DeepInf & 0.56 & 0.031 & 0.23 \\
    \hline
  \end{tabular}
  \vspace{-8pt}
\end{table}
